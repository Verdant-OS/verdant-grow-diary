  ...
  test(`CSV ${scenario.name} observations persist across the Free history window and canonical live access changes`, async ({
    ...
    try {
      ...
      const observeHiddenRetry = async () => {
        ...
        await expect(terminal).toHaveAttribute("data-testid", "csv-import-error");
        await expect(terminal).toContainText("No new CSV readings were saved in this attempt.");
        await expect(terminal).toContainText("Matching CSV history was detected");
