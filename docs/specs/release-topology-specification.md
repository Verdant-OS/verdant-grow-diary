...
| **Whether a deploy-branch production build starts a rolling release on its own in this project**                                                 | `NOT_MEASURED` as a setting | Observed: six of the eight `READY` builds after 01:27 UTC on 2026-09-26 never became a canary. The two that did each have a start event, and the 06:15 one carries no actor attribution (Appendix C). Vercel's documentation is ambiguous (§5.7). Closed when the owner reads the project's rollout-trigger setting, or when M10 before and after the next merge shows a rollout that no start event preceded |
...
| Closing read at 15:49 UTC, after C.4's hostname reads | Unchanged: `COMPLETE`, the same canary, the same `updatedAt` (06:21:02), so C.4's reads fall within one release moment |
