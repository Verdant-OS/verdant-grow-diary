...
-- Drop old signature exactly
DROP FUNCTION IF EXISTS public.quicklog_save_event(
  text, uuid, text, uuid, uuid, text, text, jsonb, timestamptz, jsonb
);

-- Recreate as the sole overload with typed payload params appended.
...
-- Restore prior EXECUTE grants exactly (anon, authenticated, service_role).
...
-- Refresh PostgREST schema cache so the new signature resolves cleanly and
-- no PGRST203 (ambiguous function) surfaces on legacy calls.
