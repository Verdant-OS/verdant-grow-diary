...
const CONFIRM_COPY = "I confirm this is my data and understand this import is not live data.";
...
function reasonCounts(
  items: ReadonlyArray<{ reasons: ReadonlyArray<string> }>,
...
export function CsvPreviewReviewGate({
  ...
  const [diaryDate, setDiaryDate] = useState<string>(() => toDateTimeLocalInputValue(effectiveNow));
  ...
  const blockedGroups = useMemo(
    () =>
      plan ? groupBlockedRowsByReason(plan.blockedRows, built?.contextByRow ?? new Map()) : [],
    [plan, built],
  );
        ...
        >
          <div className="flex flex-wrap gap-2" data-testid="csv-import-plan-counts">
            ...
            <Badge variant="secondary" data-testid="csv-import-plan-ignored">
              Ignored columns:{" "}
              {plan.ignoredUnmappedHeaders.length + plan.ignoredDeviceControlHeaders.length}
            </Badge>
          ...
          <div data-testid="csv-import-plan-blocked-reasons">
            ...
            {Object.keys(blockedReasonCounts).length === 0 ? (
              <div
                ...
                data-testid="csv-import-plan-blocked-empty"
              >
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          data-testid={`csv-import-plan-blocked-count-${g.reason}`}
                        >
                          {g.count}
                        </Badge>
                      </div>
                        ...
                        {g.samples.slice(0, BLOCKED_SAMPLE_PER_REASON_MAX).map((s, i) => (
                          <li key={i}>
                            ...
                            {s.rawValue !== undefined
                              ? ` · raw=${String(s.rawValue).slice(0, 40)}`
                              : ""}
                          </li>
                        ))}
                      </ul>
          ...
          <div data-testid="csv-import-plan-diary-summary" className="space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="space-y-1">
                <Label htmlFor="csv-gate-diary-date" className="text-xs">
                  Diary date / occurred at
                </Label>
                <Input
                ...
                <div className="flex flex-col gap-1 text-xs">
                  ...
                  {!hasExistingEntries && (
                    ...
                    >
                      ...
                      <div>
                        For now, Verdant shows the single diary summary draft that would be created
                        later.
                      </div>
            ...
            {diaryDraftDisplay ? (
              <div
                className="rounded border border-border/60 bg-muted/30 p-2 space-y-1"
                data-testid="csv-import-plan-diary-summary-card"
              >
                <div>{diaryDraftDisplay.summary}</div>
                <dl
                  className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-muted-foreground"
                  data-testid="csv-import-plan-diary-draft-fields"
                >
                  <dt>grow_id</dt>
                  <dd data-testid="diary-field-grow_id">{diaryDraftDisplay.grow_id}</dd>
                  <dt>tent_id</dt>
                  <dd data-testid="diary-field-tent_id">{diaryDraftDisplay.tent_id}</dd>
                  <dt>plant_id</dt>
                  <dd data-testid="diary-field-plant_id">{diaryDraftDisplay.plant_id ?? "—"}</dd>
                  <dt>occurred_at</dt>
                  <dd data-testid="diary-field-occurred_at">{diaryDraftDisplay.occurred_at}</dd>
                  <dt>source</dt>
                  <dd data-testid="diary-field-source">{diaryDraftDisplay.details.source}</dd>
                  <dt>import_batch_id</dt>
                  <dd data-testid="diary-field-import_batch_id">
                    {diaryDraftDisplay.details.import_batch_id.slice(0, 16)}…
                  </dd>
                  <dt>filename</dt>
                  <dd data-testid="diary-field-filename">
                    {diaryDraftDisplay.details.filename || "—"}
                  </dd>
                  <dt>row_count</dt>
                  <dd data-testid="diary-field-row_count">{diaryDraftDisplay.details.row_count}</dd>
                  <dt>accepted_count</dt>
                  <dd data-testid="diary-field-accepted_count">
                    {diaryDraftDisplay.details.accepted_count}
                  </dd>
                  <dt>blocked_count</dt>
                  <dd data-testid="diary-field-blocked_count">
                    {diaryDraftDisplay.details.blocked_count}
                  </dd>
                  <dt>duplicate_skipped_count</dt>
                  <dd data-testid="diary-field-duplicate_skipped_count">
                    {diaryDraftDisplay.details.duplicate_skipped_count}
                  </dd>
                  <dt>status</dt>
                  <dd data-testid="diary-field-status">review-only · preview</dd>
                </dl>
              </div>
            ) : (
          ...
          <div data-testid="csv-import-plan-sensor-sample" className="space-y-1">
            <div className="flex items-center justify-between gap-2">
              ...
              >
                {showSensorSample
                  ? "Hide sample sensor write drafts"
                  : "Show sample sensor write drafts"}
              </Button>
            </div>
            {sensorSample.length === 0 ? (
              <div
                className="text-muted-foreground space-y-0.5"
                data-testid="csv-gate-sensor-sample-empty"
              >
                <div>No accepted sensor write drafts to preview.</div>
                <div>
                  Fix mappings or blocked rows first, then this section will show a capped sample.
                </div>
                <div>Nothing has been saved.</div>
              </div>
            ) : !showSensorSample ? (
              <div className="text-muted-foreground" data-testid="csv-gate-sensor-sample-collapsed">
                {plan.acceptedWrites.length} draft{plan.acceptedWrites.length === 1 ? "" : "s"}{" "}
                would be created. Grouped counts shown above.
              </div>
            ) : (
              <>
                <div className="text-muted-foreground">{SAMPLE_ONLY_COPY}</div>
                <ul
                  data-testid="csv-gate-sensor-sample-list"
                  data-sample-count={sensorSample.length}
                  className="space-y-1"
                >
                  {sensorSample.map((d, i) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground">@ {d.captured_at}</span> · src=
                      {d.source} · quality={d.quality} · conf={d.raw_payload.confidence} · tent=
                      {d.tent_id}
                      {d.plant_id ? ` · plant=${d.plant_id}` : ""} · key=
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">
              JSON · report_version csv_import_plan_v1 · review-only
            </span>
          </div>
        </div>
      )}
