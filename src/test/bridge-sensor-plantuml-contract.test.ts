...
describe("bridge-sensor PlantUML architecture pack", () => {
  ...
  it("does not launder BLOCKED harness evidence as PASS", () => {
    ...
    // Reject affirmative PASS claims while allowing explicit "not/never ... PASS" guidance.
    expect(all).not.toMatch(/strict(?:(?!\b(?:not|never)\b)[^\n]){0,80}\bPASS\b/i);
    expect(all).not.toMatch(/zero-skip(?:(?!\b(?:not|never)\b)[^\n]){0,80}\bPASS\b/i);
    expect(all).not.toMatch(/BLOCKED(?:(?!\b(?:not|never)\b)[^\n]){0,40}\bas PASS\b/i);
  });
});
