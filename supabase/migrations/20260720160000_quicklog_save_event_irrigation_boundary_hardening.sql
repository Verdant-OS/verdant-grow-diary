-- quicklog_save_event — irrigation boundary hardening
--
-- Hardens the canonical Quick Log write boundary (no competing RPC). Changes vs
-- 20260707220000_quicklog_save_event_atomic_idempotency_v2.sql:
--   1. Plant/tent boundary is NULL-correct: when a plant is supplied the tent
--      must EXACTLY equal the plant's stored tent (including null). An untented
--      plant plus any non-null tent now fails closed with 'plant_not_in_tent'.
--   2. Idempotency gains a request fingerprint: quicklog_idempotency.request_hash
--      is set from the RAW request parameters. A replay of the same key with the
--      SAME request replays the original result; the same key with a DIFFERENT
--      request fails closed with 'idempotency_key_conflict' (pre-check AND the
--      concurrent race handler). Legacy / sibling rows with a null hash replay
--      permissively (unverifiable) — the guarantee is enforced for every new row.
--   3. Stricter typed-payload validation (still under 'invalid_typed_payload'):
--      unexpected keys, non-number / out-of-range measurements, oversized product
--      lists, oversized note/details, ownership-spoofing detail keys, and
--      secret-shaped detail values are rejected before any write.
--
-- Signature is unchanged (12-arg) so CREATE OR REPLACE (no DROP) suffices. The
-- pinned 3-column idempotency INSERT is preserved; the hash is set by a
-- follow-on UPDATE inside the same atomic block.
...
BEGIN
  ...
  -- Strict allowlist + range validation for the watering payload. Unexpected
  -- keys, non-number values, and out-of-range measurements are rejected. Blank
  -- is preserved as unknown (a null value simply is not a 'number').
  ...
  -- Strict allowlist + range validation for the feeding payload.
  ...
  -- Oversized note / details and ownership-spoofing or secret-shaped details are
  -- rejected before any write. Details never carries an ownership key; identity
  -- is always auth.uid().
  ...
  -- Request fingerprint over the RAW parameters (p_occurred_at incl. its NULL,
  -- and every request-distinguishing param). jsonb normalizes key order so this
  ...
  IF p_plant_id IS NOT NULL THEN
    ...
    END IF;
    -- NULL-correct: supplied tent must EXACTLY equal the plant's stored tent,
    -- including null. An untented plant plus any non-null tent fails closed.
  ...
  BEGIN
    ...
    -- Set the request fingerprint (kept out of the pinned 3-column INSERT).
  ...
  EXCEPTION WHEN unique_violation THEN
    ...
    -- idempotent response with no duplicate companion rows. A concurrent
    -- DIFFERENT request under the same key fails closed.
    SELECT grow_event_id, request_hash INTO v_existing, v_existing_hash
      FROM public.quicklog_idempotency
     WHERE user_id = uid AND idempotency_key = p_idempotency_key;
    IF NOT FOUND THEN
