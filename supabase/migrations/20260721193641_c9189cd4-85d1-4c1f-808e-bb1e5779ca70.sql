...
BEGIN
  ...
  END IF;
  IF v_subject_id IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'invalid_subject'); END IF;
  IF v_target = '' THEN RETURN jsonb_build_object('ok', false, 'reason', 'target_required'); END IF;
  ...
  v_owned := CASE v_subject_type
    ...
    ELSE false
  END;
  IF NOT v_owned THEN RETURN jsonb_build_object('ok', false, 'reason', 'subject_not_found'); END IF;
  ...
  EXCEPTION WHEN unique_violation THEN
    ...
    IF v_constraint = 'genetics_mutation_idempotency_pkey' THEN
      SELECT result, request_hash INTO v_prior, v_prior_hash
        FROM public.genetics_mutation_idempotency
        WHERE user_id = uid AND operation = v_op AND idempotency_key = p_idempotency_key;
      IF v_prior IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'idempotency_conflict'); END IF;
      IF v_prior_hash IS DISTINCT FROM v_hash THEN RETURN jsonb_build_object('ok', false, 'reason', 'idempotency_key_conflict'); END IF;
      RETURN v_prior || jsonb_build_object('reused', true);
    END IF;
    RAISE;
  END;
...
BEGIN
  ...
  SELECT * INTO ep FROM public.quarantine_episodes
    ...
    FOR UPDATE;
  IF ep.id IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'episode_not_found'); END IF;
  ...
  IF p_action = 'release' THEN
    ...
    IF scr.id IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'screening_subject_mismatch'); END IF;
    IF scr.result <> 'negative' THEN RETURN jsonb_build_object('ok', false, 'reason', 'screening_not_negative'); END IF;
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM public.genetics_screening_results WHERE supersedes_id = scr.id AND user_id = uid) THEN
    ...
    IF EXISTS (
      ...
      WHERE n.user_id = uid AND n.subject_type = ep.subject_type AND n.subject_id = ep.subject_id
        ...
        AND NOT EXISTS (SELECT 1 FROM public.genetics_screening_results s2
