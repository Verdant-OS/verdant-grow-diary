...
| Claimed applies are scoped to sandbox `bzatgtgjvuojpoxcknaa`                        | **`PASS`**           | The writeup names the sandbox project. Direct evidence of the claims' SCOPE, not of their execution |
| Claimed applies apply to knk `knkwiiywfkbqznbxwqfh`                                 | **`NOT_APPLICABLE`** | Sandbox target; the check does not apply to knk — this row is where `NOT_APPLICABLE` is true        |
| Apply this ledger to knk                                                            | **`FORBIDDEN`**      | Explicit hard stop                                                                                  |
| Preview-replay this ledger as migration                                             | **`FORBIDDEN`**      | Docs record only                                                                                    |
| Group C applied                                                                     | **`NOT_MEASURED`**   | Only the advisor writeup's deferred claim. No live-state measurement in this ledger                 |
| Apply Group C                                                                       | **`FORBIDDEN`**      | Deferred per writeup. Do not apply                                                                  |
