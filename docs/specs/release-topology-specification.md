...
**`BLOCKED`** until that platform's equivalent read is added to this table by a §15 amendment; a
Vercel read is never substituted for it, and a stale Vercel binding is never audited in its place.
...
| M5  | The second publishers' inventory and state    | **Inventory first:** list the Lovable workspace's projects (`list_projects`) and every Vercel team project (M3), and keep each one linked to this repository or its deploy branch; the known Lovable project `66255e7b-…` is one candidate, not the list. Then, for each Lovable candidate, `get_project`: `latest_commit_sha`, `is_published`, `publish_audience`, and — when the tool exposes it — the published URL. A candidate that publishes and is neither retired nor fenced is a D-RT-4 hazard. Where a listing does not expose the linked repository, the inventory is `NOT_MEASURED`, and a read of the known project alone never proves there is no other                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `BLOCKED` when a workspace or project cannot be read (no account access); `NOT_MEASURED` for the inventory when listings omit the repository link, and for the URL when a readable response omits it |
