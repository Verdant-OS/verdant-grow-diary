...
describe("normalizeEcowittVegetationTentPayload", () => {
  it("maps temp3f/humidity3/soilmoisture1 → Vegetation live snapshot", () => {
    const s = normalizeEcowittVegetationTentPayload(allTentsPayload, {
      now: NOW,
  ...
  it("missing temp3f or humidity3 degrades", () => {
    const s = normalizeEcowittVegetationTentPayload(
      { humidity3: 50, soilmoisture1: 30 },
  ...
  it("invalid soil moisture lowers root-zone confidence and marks invalid reason", () => {
    const s = normalizeEcowittVegetationTentPayload(
      { temp3f: 70, humidity3: 50, soilmoisture1: 250 },
      { now: NOW, captured_at_ms: CAPTURED },
    );
  ...
  it("never labels missing or future captured_at as live", () => {
    const missing = normalizeEcowittVegetationTentPayload(
      { temp3f: 70, humidity3: 50 },
      { now: NOW },
    );
    ...
    const future = normalizeEcowittVegetationTentPayload(
      { temp3f: 70, humidity3: 50 },
  ...
  it.each([0, 100])("rejects humidity and soil moisture rail value %s", (value) => {
    const humidity = normalizeEcowittVegetationTentPayload(
      { temp3f: 70, humidity3: value },
      { now: NOW, captured_at_ms: CAPTURED },
    );
    expect(humidity.source).toBe("invalid");
    expect(humidity.metrics.humidity_pct).toBeNull();

    const soil = normalizeEcowittVegetationTentPayload(
      { temp3f: 70, humidity3: 50, soilmoisture1: value },
      { now: NOW, captured_at_ms: CAPTURED },
    );
  ...
  it("deterministic", () => {
    const a = normalizeEcowittVegetationTentPayload(allTentsPayload, {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
    const b = normalizeEcowittVegetationTentPayload(allTentsPayload, {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
