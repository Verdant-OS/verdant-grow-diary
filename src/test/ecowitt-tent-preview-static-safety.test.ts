...
const FORBIDDEN_PATTERNS: Array<{ name: string; re: RegExp }> = [
  ...
  {
    name: "device control",
    re: /\b(deviceControl|controlDevice|setRelay|fanOn|fanOff|lightOn|lightOff|pumpOn|pumpOff)\b/,
  },
  {
    name: "diary write helper",
    re: /\b(insertDiaryEntry|createDiaryEntry|saveDiaryEntry|upsertDiaryEntry)\b/,
  },
  ...
  { name: "bridge token", re: /\b(BRIDGE_TOKEN|bridgeToken|getBridgeToken)\b/ },
  {
    ...
    re: /\b(SERVICE_ROLE|service_role_key|serviceRoleKey|SUPABASE_SERVICE_ROLE_KEY)\b/,
  },
];
