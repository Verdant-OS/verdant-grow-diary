...
HTTPS at 2026-08-21: production serves `92a983b4832e` (#1061), build
`2026-08-21T00:59:52.370Z`, provenance MATCH by recomputation; live sitemap is
**61** `<loc>` (was 56), and **"Indexable routes outside the sitemap" moves
`FAIL` → `PASS`** — five advertised and self-canonical, `/breeder-beta`
correctly absent while cross-canonicalising to `/creator-beta` and staying
`index, follow`. Closes blocker 8's sibling item.
...
values here are a FRESH measurement, not a replay of §8 — production had already
moved from `6cf3ffda0686` to `92a983b4832e` in the interval, which is the whole
argument for re-measuring before writing.

No schema, migration, or governance-file changes. Does **not** measure indexation
or migration state, and does **not** set Day 0.)
...
| Production commit                           | `PASS` — verified 2026-08-21: production serves real SHA `92a983b4832e` (#1061) (`commitSource: "git"`, `treeHash: 0ce1b69261ac…`, `dirty: false`); `scripts/resolve-release-provenance.mjs --hash=<treeHash> --ref=92a983b4832e --scan=6` returned MATCH by recomputation. Supersedes the 2026-08-20 stamp `f09febc354a4` (#1049). Note the tree hash is shared with `6cf3ffda0686` (#1062) — both are docs-only commits over an identical deployable tree, which is why the resolver also matches their release tags. Publish lags git — see the branch topology row. Single observations remain point-in-time                                                        |
| Production build time                       | `2026-08-21T00:59:52.370Z` (from the same `/version.json`; the served commit was authored `2026-08-21T00:18:00Z`). Prior live stamps `2026-08-20T18:49:50.600Z` and `2026-08-21T00:27:10.316Z` (the `6cf3ffda0686` publish, measured minutes after it landed) are historical                                                                                                                                                                                                                                                                                                                                                                                            |
