...
import {
  createCsvFixture as createLocalFixture,
  signInCsvFixture as signIn,
  type CsvFixture as LocalFixture,
} from "./lib/nativeCsvFixtures";
...
for (const scenario of [
  ...
  test(`CSV ${scenario.name} observations persist across the Free history window and canonical live access changes`, async ({
    ...
    try {
      ...
      // The merged conflict repair must identify hidden/unverified duplicates,
      // never present a generic failure or claim that old rows were not saved.
      await preview(page, f, f.primary, csv);
      const assertHiddenConflict = async () => {
        const error = page.getByTestId("csv-import-error");
        await expect(error).toContainText("Matching CSV history was detected");
        await expect(error).toContainText(
          "couldn't verify all matching readings in your current history view",
        );
        await expect(error).toContainText("Older readings may be outside that view");
        await expect(error).toContainText("retrying the same file may encounter the same conflict");
        await expect(error).toContainText("No new CSV readings were saved in this attempt");
        await expect(error).toContainText("No live sensor data was created");
        await expect(error).not.toContainText("No CSV readings were saved. Try again.");
        await expect(error).not.toContainText("23505");
        await expect(error).not.toContainText("sensor_readings_dedupe_uidx");
        await expect(page.getByTestId("csv-import-done")).toHaveCount(0);
        await expect(page.getByTestId("csv-import-retry")).toBeEnabled();
        await expect(page.getByRole("link", { name: "View imported history" })).toHaveAttribute(
          "href",
          "/tents/" + f.primary.tentId + "#imported-history",
        );
      };
      await page.getByTestId("csv-import-confirm").click();
      await assertHiddenConflict();
      const postsAfterFirstConflict = csvPosts.length;
      await page.getByTestId("csv-import-retry").click();
      await assertHiddenConflict();
      expect(csvPosts.length).toBeGreaterThan(postsAfterFirstConflict);
