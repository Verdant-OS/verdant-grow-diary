      ...
      <div className="glass relative w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto p-5 border border-border/50">
        ...
        <header className="mb-4 pr-8">
          <h2
            ...
            data-testid="timeline-evidence-drawer-title"
          >
            {vm.title}
          </h2>
          <p
            className="text-xs text-muted-foreground mt-0.5"
            data-testid="timeline-evidence-drawer-subtitle"
          >
            {vm.subtitle}
          </p>
          {vm.occurredAt && (
        ...
        <div className="flex flex-wrap gap-1.5 mb-4" data-testid="timeline-evidence-drawer-badges">
          ...
          {vm.badges.includes("sensor") && (
            <Chip icon={<Gauge className="h-3 w-3" />}>Sensor snapshot</Chip>
          )}
          {vm.badges.includes("note") && <Chip icon={<FileText className="h-3 w-3" />}>Note</Chip>}
          {vm.badges.includes("watering") && (
            <Chip icon={<Droplets className="h-3 w-3" />}>Watering</Chip>
          )}
          {vm.badges.includes("feeding") && (
            <Chip icon={<FlaskConical className="h-3 w-3" />}>Feeding</Chip>
          )}
          {vm.badges.includes("maturity_evidence") && (
            <Chip icon={<Leaf className="h-3 w-3" />}>Maturity evidence</Chip>
          )}
          {vm.badges.includes("stale_sensor") && (
            <Chip icon={<Clock className="h-3 w-3" />} tone="warn">
              Stale snapshot
            </Chip>
          )}
        </div>
        ...
        <dl className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs mb-4">
          ...
          {vm.stageLabel && (
            <Row label="Stage" value={vm.stageLabel} icon={<Sprout className="h-3 w-3" />} />
          )}
          {vm.plantLabel && (
            <Row label="Plant" value={vm.plantLabel} icon={<Leaf className="h-3 w-3" />} />
          )}
          ...
          <section className="mb-4" data-testid="timeline-maturity-evidence">
            ...
            <div className="rounded-lg border border-violet-500/30 bg-violet-500/10 p-3 text-xs text-violet-200">
              <div className="mb-2 flex flex-wrap gap-1.5">
                {vm.maturityEvidence.clearPct != null && (
                  <Chip tone="maturity">Clear {vm.maturityEvidence.clearPct}%</Chip>
                )}
                {vm.maturityEvidence.cloudyPct != null && (
                  <Chip tone="maturity">Cloudy {vm.maturityEvidence.cloudyPct}%</Chip>
                )}
                {vm.maturityEvidence.amberPct != null && (
                  <Chip tone="maturity">Amber {vm.maturityEvidence.amberPct}%</Chip>
                )}
              </div>
              ...
              {vm.maturityEvidence.advisoryOnly && (
                <p
                  className="mb-2 text-[11px] text-violet-100/90"
                  data-testid="timeline-maturity-advisory"
                >
