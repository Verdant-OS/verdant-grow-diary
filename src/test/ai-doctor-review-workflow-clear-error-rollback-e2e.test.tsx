...
describe("AI Doctor review workflow — Clear review status error rollback", () => {
  it("Clear insert is denied → existing needs_follow_up state survives end-to-end", async () => {
    ...
    const seededItems = screen.getAllByTestId("ai-doctor-session-detail-review-status-event");
    expect(seededItems.length).toBe(1);
    expect(seededItems[0].getAttribute("data-event-type")).toBe("needs_follow_up");
    ...
    await waitFor(() => expect(visibleChipPre.textContent).toBe("Needs follow-up: 1 visible"));
    ...
    await waitFor(() => expect(savedSelectPre.value).toBe(BUILTIN_SAVED_VIEW_NEEDS_FOLLOW_UP_ID));
    await waitFor(() => {
    ...
    const postItems = screen.getAllByTestId("ai-doctor-session-detail-review-status-event");
    expect(postItems.length).toBe(1);
    expect(postItems[0].getAttribute("data-event-type")).toBe("needs_follow_up");
    ...
    await waitFor(() => expect(visibleChipPost.textContent).toBe("Needs follow-up: 1 visible"));
    ...
    await waitFor(() => expect(savedSelectPost.value).toBe(BUILTIN_SAVED_VIEW_NEEDS_FOLLOW_UP_ID));
    await waitFor(() => {
      const rows = screen.getAllByTestId("ai-doctor-sessions-index-row");
      expect(rows.length).toBe(1);
    });
