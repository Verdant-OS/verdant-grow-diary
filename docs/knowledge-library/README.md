...
| `schemas/corpus-review-packet.schema.json`       | Strict shape for an evidence-sourced but still human-unreviewed corpus revision. It separates current public-source registry membership from unmeasured production publication and editorial readiness, and cannot represent this draft as ready.                                                                            |
...
- `docs/knowledge-library/corpus/pv1-symptom-evidence-guides/revisions/draft-001.json` is the first real review packet. It pins the four registered symptom-guide routes to their resolved runtime material, maps bounded claims to proposed authoritative sources, discloses AI assistance, and records no invented author, reviewer, approval, search, crawl, or publication receipt. PR #627 is route-cohort provenance only and cannot satisfy a scientific claim.
- `scripts/knowledge/validate-corpus-review-packet.mjs` validates that review packet independently of publication readiness. It derives missing-human, unrendered-source, unreviewed-differential, search-research, original-asset, AI-verification, source-authorship/date/license, claim-approval, and R2 source-mix blockers. A structural pass therefore remains `publicationReadiness: BLOCKED`; it never promotes proposed links or turns source discovery into human approval.
- `scripts/knowledge/validate-symptom-corpus-review-packet.ts` imports the real `VERDANT_SEO_GUIDES` registry, validates the packet against `schemas/corpus-review-packet.schema.json`, and compares its deterministic material digests and coverage with resolved runtime content. It reports publication and rendered-crawl status as `NOT_MEASURED`.
...
Required CI exercises both the corpus engine's synthetic adversarial suite and the real symptom review packet's fail-closed structural contract. The packet is a review input, not a publication candidate: its runtime-content receipt, source map, and derived blockers are real, while reviewer approval, repository-wide endpoint existence, editorial publication readiness, rendered HTTP/redirect/canonical parity, crawl depth, and authenticated measurement remain `NOT_MEASURED` or `BLOCKED` until their required human and delivery receipts exist.
