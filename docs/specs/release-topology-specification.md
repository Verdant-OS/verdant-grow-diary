...
| M4  | What was built, and the trigger               | List the deploy-branch first-parent commits since the last stamp (`git rev-list --first-parent <last>..<tip>`) and the Vercel `list_deployments` rows (`target=production`) for the same window, then reconcile **both ways**: every tip has exactly one READY build from `verdant-grow-diary` (a tip with none is a missing build), and every production row maps to a tip (a row that does not is an out-of-band publish, M11). Answers "was every tip built", not "what is served"                          | `BLOCKED`                                                                             |
