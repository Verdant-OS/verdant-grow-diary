...
describe("ActionQueueRpcStatusPill", () => {
  it("renders the checking placeholder while availability is unknown", () => {
    ...
    expect(
      screen.getByTestId(
        "action-queue-transition-rpc-status-pill-icon-checking",
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByTestId(
        "action-queue-transition-rpc-status-pill-icon-available",
      ),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId(
        "action-queue-transition-rpc-status-pill-icon-unavailable",
      ),
    ).not.toBeInTheDocument();
  });
  ...
  it("never leaks internal RPC identifiers into the rendered label or title", () => {
    for (const availability of [
      ...
      const { unmount } = render(
        <ActionQueueRpcStatusPill availability={availability} />,
      );
      const pill = screen.getByTestId(
        "action-queue-transition-rpc-status-pill",
      );
