...
const TRUSTWORTHY_SOURCES: ReadonlySet<Phase1SensorSourceTag> = new Set<Phase1SensorSourceTag>([
  "live",
  "manual",
]);
...
function assessPhase1Context(context: Phase1PlantContextPayload): ContextStrength {
  const trustworthyGroups = context.sensor_groups.filter((g) => TRUSTWORTHY_SOURCES.has(g.source));
