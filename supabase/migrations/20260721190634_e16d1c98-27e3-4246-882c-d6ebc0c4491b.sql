-- File contents applied verbatim from supabase/migrations/20260719075033_vpd_calibration_provenance.sql
...
BEGIN
  ...
  SELECT * INTO v_vpd FROM public.sensor_readings
   WHERE id = NEW.vpd_reading_id AND user_id = NEW.user_id AND tent_id = NEW.tent_id AND metric = 'vpd_kpa';
  IF NOT FOUND THEN RAISE EXCEPTION 'vpd_reading_id must reference the caller-owned tent VPD row'; END IF;

  SELECT * INTO v_air FROM public.sensor_readings
   WHERE id = NEW.air_temperature_reading_id AND user_id = NEW.user_id AND tent_id = NEW.tent_id AND metric = 'temperature_c';
  IF NOT FOUND THEN RAISE EXCEPTION 'air_temperature_reading_id must reference the caller-owned tent temperature row'; END IF;

  SELECT * INTO v_humidity FROM public.sensor_readings
   WHERE id = NEW.humidity_reading_id AND user_id = NEW.user_id AND tent_id = NEW.tent_id AND metric = 'humidity_pct';
  IF NOT FOUND THEN RAISE EXCEPTION 'humidity_reading_id must reference the caller-owned tent humidity row'; END IF;

  IF v_air.value < -20 OR v_air.value > 60 THEN RAISE EXCEPTION 'air temperature is outside the accepted VPD range'; END IF;
  IF v_humidity.value < 0 OR v_humidity.value > 100 THEN RAISE EXCEPTION 'humidity is outside the accepted VPD range'; END IF;
  ...
  IF abs(extract(epoch FROM (v_air_observed_at - v_humidity_observed_at))) > extract(epoch FROM interval '15 minutes')
     OR abs(extract(epoch FROM (v_vpd_observed_at - v_air_observed_at))) > extract(epoch FROM interval '15 minutes')
     OR abs(extract(epoch FROM (v_vpd_observed_at - v_humidity_observed_at))) > extract(epoch FROM interval '15 minutes') THEN
    RAISE EXCEPTION 'VPD, temperature, and humidity readings must be within 15 minutes';
  END IF;

  IF NEW.measurement_basis = 'leaf' THEN
    SELECT * INTO v_calibration FROM public.vpd_calibration_records
     WHERE id = NEW.calibration_record_id AND user_id = NEW.user_id AND tent_id = NEW.tent_id;
    IF NOT FOUND THEN RAISE EXCEPTION 'calibration_record_id must reference caller-owned tent evidence'; END IF;

    IF v_calibration.placement <> 'canopy' THEN RAISE EXCEPTION 'decision-grade leaf VPD requires canopy placement'; END IF;
    ...
    END IF;
    IF v_calibration.temperature_verified_at > v_evaluated_at OR v_calibration.humidity_verified_at > v_evaluated_at THEN
      RAISE EXCEPTION 'calibration verification timestamps cannot be in the future';
    END IF;
    IF v_calibration.humidity_reference_rh_pct < 75 OR v_calibration.humidity_reference_rh_pct > 100 THEN
    ...
    END IF;
    IF v_calibration.sensor_commissioned_at IS NOT NULL AND v_calibration.sensor_commissioned_at > v_vpd_observed_at THEN
      RAISE EXCEPTION 'sensor commissioned date cannot be after the observation';
    END IF;
    IF NULLIF(btrim(v_air.device_id), '') IS NULL OR v_air.device_id <> v_calibration.device_id THEN
      RAISE EXCEPTION 'temperature reading device does not match calibration evidence';
    END IF;
    IF NULLIF(btrim(v_humidity.device_id), '') IS NULL OR v_humidity.device_id <> v_calibration.device_id THEN
    ...
    END IF;
    IF v_vpd.source NOT IN ('live','manual','csv','pi_bridge','webhook_generic','node_red_bridge','esp32_arduino','esp32_arduino_sht31','esp32_esphome','esp32_mqtt_bridge','home_assistant_bridge','ha_forwarded','ecowitt','mqtt','webhook')
       OR v_air.source NOT IN ('live','manual','csv','pi_bridge','webhook_generic','node_red_bridge','esp32_arduino','esp32_arduino_sht31','esp32_esphome','esp32_mqtt_bridge','home_assistant_bridge','ha_forwarded','ecowitt','mqtt','webhook')
       OR v_humidity.source NOT IN ('live','manual','csv','pi_bridge','webhook_generic','node_red_bridge','esp32_arduino','esp32_arduino_sht31','esp32_esphome','esp32_mqtt_bridge','home_assistant_bridge','ha_forwarded','ecowitt','mqtt','webhook') THEN
    ...
    END IF;
    IF abs(extract(epoch FROM (NEW.leaf_temperature_measured_at - v_air_observed_at))) > extract(epoch FROM interval '15 minutes')
       OR abs(extract(epoch FROM (NEW.leaf_temperature_measured_at - v_humidity_observed_at))) > extract(epoch FROM interval '15 minutes') THEN
    ...
    v_corrected_air_temp_c := v_air.value + v_calibration.temperature_correction_offset_c;
    v_corrected_humidity_pct := v_humidity.value + v_calibration.humidity_correction_offset_pct;
    ...
    v_air_saturation_kpa := 0.6108 * exp((17.27 * v_corrected_air_temp_c) / (v_corrected_air_temp_c + 237.3));
    v_leaf_saturation_kpa := 0.6108 * exp((17.27 * NEW.leaf_temperature_c) / (NEW.leaf_temperature_c + 237.3));
    v_expected_vpd := v_leaf_saturation_kpa - (v_air_saturation_kpa * v_corrected_humidity_pct / 100);
...
CREATE POLICY "Users view own VPD calibration records"
  ON public.vpd_calibration_records FOR SELECT TO authenticated
  USING ((select auth.uid()) = user_id);

CREATE POLICY "Users insert own VPD calibration records"
  ON public.vpd_calibration_records FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() = user_id
    ...
    AND EXISTS (SELECT 1 FROM public.tents WHERE id = tent_id AND user_id = auth.uid())
  );

CREATE POLICY "Users view own VPD measurement provenance"
  ON public.vpd_measurement_provenance FOR SELECT TO authenticated
  USING ((select auth.uid()) = user_id);

CREATE POLICY "Users insert own VPD measurement provenance"
  ON public.vpd_measurement_provenance FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() = user_id
    AND recorded_at >= now() - interval '5 minutes'
    AND recorded_at <= now() + interval '5 minutes'
    AND EXISTS (SELECT 1 FROM public.tents WHERE id = tent_id AND user_id = auth.uid())
    AND EXISTS (SELECT 1 FROM public.sensor_readings WHERE id = vpd_reading_id AND user_id = auth.uid() AND tent_id = vpd_measurement_provenance.tent_id AND metric = 'vpd_kpa')
    AND EXISTS (SELECT 1 FROM public.sensor_readings WHERE id = air_temperature_reading_id AND user_id = auth.uid() AND tent_id = vpd_measurement_provenance.tent_id AND metric = 'temperature_c')
    AND EXISTS (SELECT 1 FROM public.sensor_readings WHERE id = humidity_reading_id AND user_id = auth.uid() AND tent_id = vpd_measurement_provenance.tent_id AND metric = 'humidity_pct')
    AND (
      calibration_record_id IS NULL
      OR EXISTS (SELECT 1 FROM public.vpd_calibration_records WHERE id = calibration_record_id AND user_id = auth.uid() AND tent_id = vpd_measurement_provenance.tent_id)
    )
  );

INSERT INTO supabase_migrations.schema_migrations (version, name, statements)
VALUES ('20260719075033', 'vpd_calibration_provenance', ARRAY['-- applied via lovable per-file chain'])
