...
describe("CustomerGuideTrustFooter", () => {
  it("renders the privacy, telemetry, and backend copy", () => {
    ...
    expect(screen.getByTestId("customer-guide-trust-privacy")).toHaveTextContent(
      /private grow logs, sensor payloads, raw payloads, and operator notes are not shown\./i,
    );
    expect(screen.getByTestId("customer-guide-trust-telemetry")).toHaveTextContent(
      /this page is not live sensor telemetry\./i,
    );
    expect(screen.getByTestId("customer-guide-trust-backend")).toHaveTextContent(
  ...
  it("renders the five required FAQ questions", () => {
    ...
    for (const id of [
      ...
      expect(screen.getByTestId(`customer-guide-trust-faq-${id}`)).toBeInTheDocument();
    }
  });
  ...
  it("does NOT make medical/compliance/potency/lab claims", () => {
    ...
    expect(text).not.toMatch(
      /\b(medical|FDA|HIPAA|GDPR|lab[\s-]?tested|potency|THC %|CBD %|cure[ds]?\b|treat[s]?\b)\b/i,
    );
  });
});
