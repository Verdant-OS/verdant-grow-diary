...
- Screening and quarantine records are immutable events. A correction adds a later-recorded event that supersedes the original and states why; a retest adds later-recorded independent evidence and retains both results. A correction or retest cannot carry collection, result, or record chronology earlier than the event it references. Conflicting or discordant results remain visible.
...
- Quarantine uses explicit `open`, `release`, `dispose`, `reopen`, and `override` events. Normal release requires current, unsuperseded, scope-matching negative evidence demonstrably collected after the latest open/reopen and recorded no later than the release event; evidence learned later cannot justify an earlier release. Because `collectedOn` is date-only, evidence from the same UTC calendar date as the latest open/reopen is ambiguous and cannot support normal release. Contradictory evidence available at release blocks release. An override is labeled as an override and never presented as laboratory clearance.
