 ...
 * "AI Doctor credits used up" retention/conversion marker.
 *
 * Free-plan grows get a fixed, non-renewing AI Doctor credit allotment
 * (aiCreditsPerGrow) — unlike Pro's monthly-renewing pool. Once that
 * allotment is spent for THIS grow, the doctor surfaces themselves stay
 * silent about it (they intentionally show no paywall/upgrade copy — see
 * PlantDetailAiDoctorLiveReview's plan-neutral denial notice). This module
 * is for a calm, separate, Plant-Detail-level nudge shown at the exact
 * moment the grower has already demonstrated demand for the feature.
 *
 ...
 *     own resolved limit — never a guess, never an estimate.
...
export function buildAiDoctorCreditsExhaustedTeaserView(
  ...
  const resolved =
    isFiniteNonNegative(input.limit) &&
    input.limit > 0 &&
  ...
  const show = input.isFreePlan === true && remaining <= 0;
