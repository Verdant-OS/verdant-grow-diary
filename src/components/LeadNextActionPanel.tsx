 ...
 */
export default function LeadNextActionPanel({ lead }: LeadNextActionPanelProps) {
  if (!lead) {
    return (
      <p className="text-xs text-muted-foreground" data-testid="lead-next-action-empty">
        No lead selected.
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      {rec.warning && (
        <p className="text-xs text-destructive" data-testid="lead-next-action-warning">
          {rec.warning}
        </p>
      )}
    </div>
