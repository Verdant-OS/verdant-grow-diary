-- Widen grow_events validation trigger to allow harvest + cure_check.
-- Pre-existing allowed types are preserved verbatim; nothing is removed.
...
-- Widen quicklog_save_event RPC whitelist to match the trigger.
-- All other behavior, parameters, RLS, grants, and audit logic are
-- preserved exactly as the prior version.
...
BEGIN
  ...
  -- Trigger-aligned list. 'harvest' and 'cure_check' are accepted as
  -- V0 harvest/cure persistence-contract event types. Note-style Quick
  -- Logs still arrive as event_type=observation with p_details.kind=note.
...
COMMENT ON FUNCTION public.quicklog_save_event(text, uuid, text, uuid, uuid, text, text, jsonb, timestamptz, jsonb) IS
  'Atomic Quick Log writer (v2): event-type whitelist now includes harvest and cure_check for V0 harvest/cure persistence-contract slice. Note-style Quick Logs still arrive as event_type=observation with p_details.kind=note. Trigger-aligned event-type whitelist. Sensor snapshot provenance preserved verbatim.';