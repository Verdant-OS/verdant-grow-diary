...
export const MCP_MANIFEST: MCPManifestView = Object.freeze({
  ...
  tools: [
    ...
    },
    {
      ...
      description:
        "List the signed-in Verdant grower's own tents and plants within one owned grow, ordered by deterministic physical-inspection priority. Results preserve missing evidence and source limits; priority is scouting guidance, not a diagnosis. Read-only.",
      readOnly: true,
      params: [
        {
          name: "growId",
    ...
    },
    {
      ...
      description:
        "Fetch bounded, source-labeled evidence for one tent or plant the signed-in grower owns. Photo rows are metadata only, sensor evidence keeps source/quality/freshness labels, and partial lanes are named explicitly. The result supports physical inspection and does not diagnose, approve actions, or control equipment. Read-only.",
      readOnly: true,
      params: [
        {
          name: "targetType",
