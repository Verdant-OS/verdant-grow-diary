...
describe("Grow Walk MCP tools", () => {
  ...
  it("returns the standard authentication error without constructing a user client", async () => {
    await expect(
      listGrowWalkTargetsTool.handler({ growId: "4d93e80b-9656-413c-bceb-d2f6fb0fdf45" }, ctx(false)),
    ).resolves.toEqual({
  ...
  it("returns calm fail-closed errors without leaking database detail", async () => {
    ...
    const contextResult = await getGrowWalkContextTool.handler(
      { targetType: "tent", targetId: "0b73545b-00ef-4381-94f9-27c82a8d5a83" },
      ctx(true),
    );
    ...
    expect(contextResult).toEqual({
      content: [
        { type: "text", text: "Grow Walk target not found for the signed-in grower." },
      ],
      isError: true,
    });
  });
