-- Forward-only repair for deployed databases that drifted past two core
-- column migrations while the application already depended on them.
--
-- This migration intentionally sorts before the dual-timestamp foundation.
-- It does not edit or mark historical migrations as applied.
--
-- quicklog_idempotency.request_hash stays nullable: legacy rows cannot be
-- assigned a truthful hash retroactively. Every new canonical Quick Log save
-- receives a hash from the RPC definition reinstalled below.
ALTER TABLE public.quicklog_idempotency
  ADD COLUMN IF NOT EXISTS request_hash text;

-- PostgreSQL fast-defaults the confirmed missing-column path without rewriting
-- plant rows or firing their update triggers. The normalization below remains
-- necessary for databases where the column exists but its contract drifted.
...
SET plant_type = CASE
  WHEN lower(btrim(COALESCE(plant_type, ''))) IN ('autoflower', 'photoperiod', 'unknown')
    THEN lower(btrim(plant_type))
  ELSE 'unknown'
END
WHERE plant_type IS DISTINCT FROM CASE
  WHEN lower(btrim(COALESCE(plant_type, ''))) IN ('autoflower', 'photoperiod', 'unknown')
    THEN lower(btrim(plant_type))
  ELSE 'unknown'
END;
...
ALTER TABLE public.plants
  VALIDATE CONSTRAINT plants_plant_type_check;
