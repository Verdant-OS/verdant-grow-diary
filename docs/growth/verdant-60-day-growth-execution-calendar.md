...
| CSV acquisition   | `csv_history_onboarding_ready -> csv_import_started -> csv_import_completed`       | Readiness means editable starter setup completed and the tent-scoped importer is shown; an activated import requires `rows > 0`. Retain zero-row completions as no-op diagnostics. |
