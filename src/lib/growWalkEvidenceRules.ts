...
function latestIso(values: readonly { readonly iso: string; readonly ms: number }[]): string | null {
...
function orderedCodes<T extends string>(allowed: readonly T[], found: ReadonlySet<T>): readonly T[] {
...
function isOpenAlert(alert: GrowWalkAlertEvidence): boolean {
...
/**
 * Derive source-labeled scouting evidence only. This function never diagnoses,
 * writes, invokes AI, mutates alerts, changes Action Queue state, or controls devices.
 */
export function deriveGrowWalkEvidence(input: GrowWalkEvidenceInput): GrowWalkEvidenceDerivation {
  ...
  if (input.aiDoctor) {
    const aiMs = toMs(input.aiDoctor.completedAt);
    if (!isUsableTimestamp(aiMs, nowMs)) {
      contradictions.add("future_or_malformed_timestamp");
    }
  }
  ...
  for (const reading of readings) {
    const ms = toMs(readingTimestamp(reading));
    if (!isUsableTimestamp(ms, nowMs)) {
      contradictions.add("future_or_malformed_timestamp");
    }
  }
  ...
  const majorChanges = validEvents.filter(
    ({ event, ms }) =>
      event.isMajorChange && ms <= nowMs && nowMs - ms <= MAJOR_CHANGE_WINDOW_MS,
  );
  ...
  // A stored photo-event is metadata only. No image content was inspected in this derivation.
  ...
  const openHighAlerts = validAlerts.filter(
    ({ alert }) => alert.severity === "high" && isOpenAlert(alert),
  );
  if (openHighAlerts.length > 0) reasons.add("active_high_alert_needs_confirmation");

  const stage = normalize(input.stage);
  if (
    stage.includes("flower") &&
    openHighAlerts.some(({ alert }) => isHumidityAlert(alert))
  ) {
    reasons.add("flower_humidity_alert_needs_inspection");
  }

  const hasProblemPeriod =
    openHighAlerts.length > 0 || recentMajorChangeCount48h > 0 || worsening.length > 0;
  ...
  const adverseTimestamps = [
    ...worsening,
    ...openHighAlerts,
  ].map(({ iso, ms }) => ({ iso, ms }));
  const latestAdverseEvidenceAt = latestIso(adverseTimestamps);
