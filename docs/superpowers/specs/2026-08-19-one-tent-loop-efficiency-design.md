...
- **Check-ins only, never merged activity.** `useDashboardScopedData` merges
  ...
  prompt still shows".
- **Plant intent on grow-scoped surfaces is explicit or provable, never
  guessed.** Dashboard and Grow Detail have no selected plant. The CTA
  passes a plant prefill **only** when the scoped grow has exactly one
  active plant (the sanctioned TentDetail `safePlantId` sole-plant
  precedent — a page-owned derivation, not dialog self-defaulting; the
  `quicklog-plant-default` fence stays green). Multi-plant grows open the
  dialog unscoped, where the grower makes one explicit plant choice (the D5
  suggestion chip, once merged, reduces that to one tap).
...
| **B3a** | Recovery parity on Dashboard + Grow Detail by extending the SHIPPED `noRecentLogRecoveryRules.ts` (D-B5, corrected twice) — no new rules engine                                        | `src/pages/Dashboard.tsx:1543`, `src/pages/GrowDetail.tsx:306` (empty states consume `buildNoRecentLogRecovery` over a **check-in-only projection** + CTA opens that surface's existing Quick Log entry, sole-active-plant prefill else unscoped)   | `src/lib/recoveryCheckInProjection.ts` (pure filter, not an engine) + test                                             | Prompt renders on no/stale **check-in** with the shipped copy; **recent Action Queue event + no recent check-in → prompt still shows** (the false-negative regression); recent check-in → no prompt; sole-plant grow → CTA carries plant prefill, multi-plant → unscoped (no auto-selection; `quicklog-plant-default` fence green); CTA opens Quick Log (no write); existing `no-recent-log-recovery-rules` + recap suites untouched and green | nothing (pages unowned by A2–A5; hooks NOT edited)                                                                                                                            |
...
| S6 recovery                          | SHIPPED on Plant Detail (3 taps, 72 h window); `MISSING` on Dashboard/Grow Detail | **3 taps where a plant is provable** (Plant Detail always; Dashboard/Grow Detail when the scoped grow has exactly one active plant); **3 taps + one explicit plant choice otherwise** (the D5 chip is that one choice); check-in-only classification — never suppressed by Action Queue/alert activity; no guilt copy, no forced note/sensor/AI | B3a (+D5 for the one-tap choice)                |
