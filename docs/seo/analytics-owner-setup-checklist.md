...
| Enhanced measurement review                   | `BLOCKED_BY_ACCESS` | In the existing production stream, open Enhanced Measurement > Page views > advanced settings and disable **Page changes based on browser history events**. Retain Verdant's explicit SPA `page_view` emitter; do not create a new stream. | Repeat direct load, cross-guide navigation, back, forward, refresh, repeated navigation, and new-tab checks with collection blocked locally. |
