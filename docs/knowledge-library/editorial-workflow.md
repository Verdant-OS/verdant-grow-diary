...
Before the page-revision workflow begins, a first-500 roadmap candidate follows the smaller brief lifecycle `needs_editorial_brief -> draft -> reviewed`. Its link brief follows `needs_review -> draft -> reviewed`, and its search brief follows `needs_research -> draft -> validated`. Required CI compares these fields with the exact target/base roadmap: no lane may skip a state or regress. Once a lane reaches its final state, its authored brief, reviewed links, or validated search evidence is frozen against in-place rewriting; later change requires a separately governed append-only amendment contract. The identity/allocation digest remains unchanged because these editorial states are mutable operating metadata, not canonical allocation.
...
`roadmap-500.json` is an immutable v1 identity/allocation baseline and therefore remains provisional. Its digest pins exactly `id`, `path`, `priority`, `wave`, `pillar`, `pillarRank`, `parentPath`, and `pageFamily`; it deliberately excludes later research receipts, briefs, priority signals, collision review, route state, its derived priority lane, and readiness. It is never rewritten to pretend later research happened. `intent-research-registry.json` stores append-only lifecycle events and receipts outside that baseline. Its derived state becomes `researched` only when the receipt above exists. Post-publication query/task evidence may advance it to `validated`; later intent drift appends a `superseded` or return-to-`provisional` event. Required CI compares this history with the exact target/base revision; local review may use explicit previous/current files. Search demand is a prioritization signal, never evidence for a cultivation claim and never grounds for delaying urgent safety, prerequisite, or correction work.
