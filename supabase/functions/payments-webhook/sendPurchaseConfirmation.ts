...
export async function maybeSendPurchaseConfirmation(
  ...
  try {
    if (!evt?.eventType || !CONFIRMATION_EVENT_TYPES.has(evt.eventType)) {
      return logSkip(
        { reason: 'skipped:event_type_not_confirmable', detail: evt?.eventType },
        evt,
      );
    }
    ...
    try {
      ...
      return logSkip(
        { reason: 'skipped:user_lookup_failed', detail: String(e) },
        evt,
      );
    }
    if (userRes.error) {
      return logSkip(
        { reason: 'skipped:user_lookup_failed', detail: userRes.error.message },
        evt,
      );
    }
    ...
    const firstName =
      (userRes.data.user.user_metadata as Record<string, unknown> | undefined)?.first_name as
        | string
        | undefined;
    ...
    if (!resp.ok) {
      return logSkip(
        { reason: 'skipped:send_non_2xx', detail: `status=${resp.status}` },
        evt,
      );
    }
    return { reason: 'sent' };
    ...
    return logSkip(
      { reason: 'skipped:dispatch_threw', detail: String(err) },
      evt,
    );
  }
}
