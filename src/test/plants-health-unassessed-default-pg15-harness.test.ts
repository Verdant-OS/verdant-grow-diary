...
const HARNESS_PATH = resolve("scripts/run-plants-health-unassessed-default-pg15-harness.mjs");
const RUNNER_PATH = resolve("scripts/apply-plants-health-unassessed-default.mjs");
const WORKFLOW_PATH = resolve(".github/workflows/plants-health-unassessed-default-pg15.yml");
...
const DISPOSABLE_DATABASE_URL =
  "postgresql://postgres:verdant-runtime-only@127.0.0.1:5432/verdant_plants_health_default";
...
describe("plants health unassessed default PostgreSQL 15 runtime gate", () => {
  ...
  it("rejects remote and arbitrary loopback targets before spawning psql", async () => {
    ...
    for (const databaseUrl of [
      ...
      "postgresql://service_role:secret@127.0.0.1:5432/verdant_plants_health_default",
      "postgresql://postgres:secret@127.0.0.1:6543/verdant_plants_health_default",
      "postgresql://postgres:secret@127.0.0.1:5432/postgres",
      "postgresql://postgres:@127.0.0.1:5432/verdant_plants_health_default",
      undefined,
    ]) {
  ...
  it("builds the production baseline in one transaction: scaffold, reviewed prerequisite, seed", async () => {
    ...
    expect(baseline).toContain(
      readFileSync(
        resolve("supabase/migrations/20260516204601_eb069c76-870d-4f19-8d23-800be7bbfe01.sql"),
        "utf8",
      ),
    );
    ...
    // Supabase-style creation defaults, so the prerequisite's objects get client grants.
    expect(baseline).toContain("grant all on functions to anon, authenticated, service_role");
  ...
  it("drives every drift class through the delivery preflight, not a local copy", async () => {
    ...
    expect(labels).toEqual(
      expect.arrayContaining([
        "health_check_constraint",
        "other_health_trigger",
        "health_policy",
        "validate_trigger_dropped",
        "validate_trigger_disabled",
        "validate_function_security_definer",
        "validate_function_search_path",
        "validate_function_overload",
        "health_column_nullable",
        "plants_rls_disabled",
        "legacy_body_drift",
        "health_default_drift",
        "partial_default_only",
        "delivered_comment_drift",
        "delivered_body_drift",
        "health_check_constraint_after_delivery",
    ...
    // Every way something else could reject 'unknown' is a prerequisite stop.
    for (const label of ["health_check_constraint", "other_health_trigger", "health_policy"]) {
      expect(cases.find((drift) => drift.label === label)?.expected, label).toBe(
        "prerequisite_drift",
      );
    }
  ...
  it("is path-scoped to a pinned PostgreSQL 15.18 loopback service", () => {
    ...
    expect(trigger.pull_request.paths).toContain(
      "src/test/plants-health-unassessed-default-pg15-harness.test.ts",
    );
    ...
    expect(job.env.PLANTS_HEALTH_DEFAULT_PG15_URL).toBe(DISPOSABLE_DATABASE_URL);
  ...
  it("initializes the exact disposable sentinel in one transaction", () => {
    ...
    expect(markerStep.run).toContain(
      "create schema verdant_plants_health_default_harness authorization postgres",
    );
