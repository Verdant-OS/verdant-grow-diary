...
describe("QuickLog component wiring", () => {
  ...
  it("does not gate save on validation warnings (submit not blocked)", () => {
    ...
    // a known unresolved Watering, and missing required plant context (Gate 1
    // bug fix: visible picker and save validator must agree). It must NOT be
    // gated on soft preview/validation warnings.
    expect(QUICKLOG).toMatch(/const saveLocked = busy \|\| childSaveBusy/);
    expect(QUICKLOG).toMatch(
      /disabled=\{saveLocked\s*\|\|\s*recoveryLocked\s*\|\|\s*!resolvedTarget\s*\|\|\s*!!savedTarget\}/,
    );
