...
describe("HarvestEvidenceReportExportButton", () => {
  it("renders the Export PDF button", () => {
    render(<HarvestEvidenceReportExportButton report={EMPTY_REPORT} now={NOW} />);
  ...
  it("is disabled while loading", () => {
    render(<HarvestEvidenceReportExportButton report={EMPTY_REPORT} isLoading now={NOW} />);
    expect(screen.getByTestId("harvest-evidence-report-export-button")).toBeDisabled();
  });

  it("is enabled when report has evidence", () => {
    render(<HarvestEvidenceReportExportButton report={POPULATED_REPORT} now={NOW} />);
    expect(screen.getByTestId("harvest-evidence-report-export-button")).not.toBeDisabled();
  });

  it("invokes window.print() on click", () => {
    render(<HarvestEvidenceReportExportButton report={POPULATED_REPORT} now={NOW} />);
    fireEvent.click(screen.getByTestId("harvest-evidence-report-export-button"));
    expect(window.print).toHaveBeenCalledTimes(1);
  });

  it("renders required print-section content (title, generated, scope, totals, footer)", () => {
    render(<HarvestEvidenceReportExportButton report={POPULATED_REPORT} now={NOW} />);
    const section = screen.getByTestId("harvest-evidence-report-print-section");
    expect(section).toHaveAttribute("data-print-section", "harvest-evidence-report");
    expect(section.textContent ?? "").toMatch(/Harvest Evidence Report/);
    expect(
      screen.getByTestId("harvest-evidence-report-print-generated-at").textContent ?? "",
    ).toMatch(/2026-06-18T12:34:56Z/);
    expect(screen.getByTestId("harvest-evidence-report-print-scope").textContent ?? "").toMatch(
      /Sour Diesel/,
    );
    expect(screen.getByTestId("harvest-evidence-report-print-totals").textContent ?? "").toMatch(
      /Trichome inspections:/,
    );
    expect(screen.getByTestId("harvest-evidence-report-print-caution").textContent ?? "").toMatch(
      /Harvest Evidence Report is diary evidence only/,
    );
    expect(
      screen.getByTestId("harvest-evidence-report-print-no-actions").textContent ?? "",
    ).toMatch(/does not create alerts, Action Queue items, or harvest instructions/);
    expect(screen.getByTestId("harvest-evidence-report-print-footer").textContent ?? "").toMatch(
      /Generated by Verdant/,
    );
  });

  it("renders empty-evidence state when no evidence present", () => {
    render(<HarvestEvidenceReportExportButton report={EMPTY_REPORT} now={NOW} />);
    expect(screen.getByTestId("harvest-evidence-report-print-empty").textContent ?? "").toMatch(
  ...
  it("print section never includes plant_id / grow_id / tent_id / user_id / raw_payload / sensor_readings text", () => {
    render(<HarvestEvidenceReportExportButton report={POPULATED_REPORT} now={NOW} />);
    const text = screen.getByTestId("harvest-evidence-report-print-section").textContent ?? "";
