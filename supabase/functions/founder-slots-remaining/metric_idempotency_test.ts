...
Deno.test(
  "request_metric row carries a deterministic idempotency_key derived from fn + request_id",
  async () => {
    primeEnv();
...
Deno.test(
  "repeated persist calls for the same request_id emit the same idempotency_key so the unique index dedups them",
  async () => {
    primeEnv();
    const rows: Record<string, unknown>[] = [];
    __setMetricPersistorForTesting(async (row) => {
  ...
  async () => {
    ...
    try {
      ...
      for (const snap of snapshots) {
        ...
        assert(
          /^\d+$/.test(suffix),
          `snapshot key suffix must be a ms timestamp — got '${suffix}'`,
        );
      }
    } finally {
      reset();
