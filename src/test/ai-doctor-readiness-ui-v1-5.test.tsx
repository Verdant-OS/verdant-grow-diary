...
describe("AI Doctor Readiness UI v1.5 — source badge edge cases", () => {
  ...
  it("panel: with zero badges, sources list collapses to safe 'No sensor data' note (no fake live)", () => {
    ...
    const note = screen.getByTestId("ai-doctor-context-readiness-panel-no-sources");
    ...
    expect(screen.queryByTestId("ai-doctor-context-readiness-panel-sources")).toBeNull();
  });
  ...
  it("panel: every rendered source badge carries sampleCount >= 1", () => {
    ...
    render(<AiDoctorContextReadinessPanel context={context} />);
    const list = screen.getByTestId("ai-doctor-context-readiness-panel-sources");
  ...
  it("mixed set: trusted (live/manual) keep emerald palette; untrusted (csv/demo/stale/invalid) keep amber palette", () => {
    ...
    for (const cse of SOURCE_BADGE_CASES) {
      const badge = screen.queryByTestId(`ai-doctor-context-readiness-panel-source-${cse.source}`);
  ...
  it("compiler collapses unknown/missing rawSource into the invalid badge; never trusted, never 'live'", () => {
    const unknownReadings = [
      // Unknown vendor string
      {
        ...
        value: 24,
        captured_at: new Date("2026-06-10T11:00:00Z").toISOString(),
        source: "totally_unknown_vendor",
      },
      // Missing source
      {
        metric: "humidity_pct",
    ...
    const invalid = screen.getByTestId("ai-doctor-context-readiness-panel-source-invalid");
    expect(invalid.getAttribute("data-trustworthy")).toBe("false");
    expect(CAUTION_TONE_CLASSES.every((c) => invalid.className.includes(c))).toBe(true);
    expect(TRUSTED_TONE_CLASSES.every((c) => invalid.className.includes(c))).toBe(false);
...
function readHeaders(testId: string): string[] {
  ...
  return Array.from(root.querySelectorAll("h2, h3")).map((h) => (h.textContent ?? "").trim());
}
...
describe("AI Doctor Readiness UI v1.5 — missing context: desktop + mobile parity", () => {
  ...
  for (const vp of [
    ...
    it(`${vp.name}: missing-context panel renders deterministic headers + quick actions`, () => {
      ...
      expect(readHeaders("ai-doctor-context-readiness-panel")).toEqual(EXPECTED_MISSING_HEADERS);
...
describe("AI Doctor Readiness UI v1.5 — mount fallback states: desktop + mobile parity", () => {
  ...
  for (const vp of [
    ...
    it(`${vp.name}: loading state renders safe checking copy and no panel`, () => {
      ...
      const loading = screen.getByTestId("plant-detail-ai-doctor-context-readiness-mount-loading");
      expect(loading.textContent ?? "").toContain("Checking AI Doctor context");
      assertNoForbiddenCopy("plant-detail-ai-doctor-context-readiness-mount-loading");
    ...
    it(`${vp.name}: empty plantId renders fallback "not available" copy`, () => {
      setViewport(vp.w, vp.h);
      render(<PlantDetailAiDoctorContextReadinessMount {...baseMountProps} plantId="" />);
      const empty = screen.getByTestId("plant-detail-ai-doctor-context-readiness-mount-empty");
      expect(empty.textContent ?? "").toContain("AI Doctor context is not available");
      assertNoForbiddenCopy("plant-detail-ai-doctor-context-readiness-mount-empty");
      expect(screen.queryByTestId("ai-doctor-context-readiness-panel")).toBeNull();
    });
    ...
    it(`${vp.name}: compile-error (malformed manual log) renders safe fallback, never the panel`, () => {
      ...
      expect(fallback.textContent ?? "").toContain("AI Doctor context is not available");
      assertNoForbiddenCopy("plant-detail-ai-doctor-context-readiness-mount-fallback");
      expect(screen.queryByTestId("ai-doctor-context-readiness-panel")).toBeNull();
      expect(fetchSpy).not.toHaveBeenCalled();
    });
  }
