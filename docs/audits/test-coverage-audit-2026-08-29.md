...
- **Breadth is genuinely good.** **3,008 test files across the four test lanes** — 2,908 Vitest, 60
  ...
  **21 `node:test` files under `scripts/`**, 19 of them referenced by a workflow or a `package.json`
  ...
  as those five measured groupings. See §9.0 defect 29.
...
- **Executed-case results** — the 39,407 discovered / 39,217 executed / 190 skipped in §9, and every
  ...
  `it()`/`test()` call-site total (31,965) counts registration calls in source and is **not** the
...
| Module reachability         | **Runtime** import graph from `@/…` and relative specifiers, walked **per test file** with that test's factory-mocked modules blocked at every depth, then unioned. Imports are read from the **TypeScript compiler's own AST**, not regex-matched. `import type` / `export type`, fully type-only clauses, `import()` in a **type** position and `.d.ts` files are erased by the transpiler and excluded; `vi.importActual`, the factory's `importOriginal` callback and a bare `vi.mock` load the real module and count, a `vi.mock` with a factory does not |
...
4. **Static call-site counts are not executed-case counts.** The per-bucket tables in §3 count
   ...
   cases and **executes 39,217** of them (190 skipped), against **31,965** such call sites — the
...
execute no repository module and render nothing — not `src/`, not `scripts/`, not
`supabase/functions/` — so nothing in them observes runtime behaviour.

### 3.1 The measurement

| Bucket                                                                            | Files             | Assertions         | Cases             |
| --------------------------------------------------------------------------------- | ----------------- | ------------------ | ----------------- |
| **Scan-only** (reads files; executes no repository module; never renders)         | **534** (18.4%)   | **13,246** (15.2%) | **4,490** (14.0%) |
| Hybrid (scans files _and_ imports/renders)                                        | 872               | 35,410             | 12,948            |
...
Within the scan-only bucket, **85.0% of assertions are `toContain(...)` or `toMatch(...)`** —
11,253 substring and regex checks over file text, which is **12.9% of all 87,333 bare
`expect(...)` calls**, not 15.2%. (15.2% is the bucket's total share of assertions; the two figures were
conflated in an earlier draft of §0.) Precisely, that count is **`toContain`/`toMatch` calls inside
scan-only files**; it does not trace each matcher's receiver back to file content. Measured, **1 of
the 11,253** asserts membership in a literal array rather than matching text — `expect(["live",
"manual", …]).toContain(p.source)` in `one-tent-loop-safety-regression.test.ts`. It was 4 of 14,054
when the bucket was wider; defect 28 moved the other three out. 0.01%, so the 12.9% headline is unchanged
...
2. **Baseline the existing 534** into an allowlist so only _new_ violations fail. Shrink the
...
| 27  | A registrar selected at **runtime** — `(cond ? it : it.skip)(…)`, or bound to an identifier — returned no chain root, so the case vanished                                             | case sites **31,941 → 31,965**; scan-only cases **5,641 → 5,665** before defect 28 moved them again                                                                                                   |
...
so reachability held at 2,061 of 2,106 as it then stood. Eight parser cases pin the two directions.
...
With 13 fixed, the direct/transitive split moves **1,636/425 → 1,637/424**. Total reached was
unchanged at 2,061 of 2,106 at that point, so the published reachability held either way — the correction moves a module
...
Defect 18 is the second pattern once more, and the narrowest instance of it: the fix for 13 used the
bypass set only to _veto a subtraction_, so a module mocked with `importOriginal` and imported only
as a type — or not imported at all — still emitted no edge, even though the rule this section
publishes says the real module loaded. A bypass is now an edge in its own right. Measured, it moves
**no** module: each such module is already directly imported by another test file. The code and the
published rule had disagreed, which in this document is the defect regardless of the arithmetic.

**Defects 19 and 20 are neither pattern — they are simply things nobody looked at.** `.d.ts` files
were counted as product modules although the transpiler erases them and they can never carry a
runtime edge, so `src/types/global-jsx.d.ts` and `src/types/mjs-modules.d.ts` sat permanently in both
the denominator and the unreached list: **2,108 → 2,106**, unreached **47 → 45**, reachability
**97.8% → 97.9%**. And `testFilesAcrossAllLanes` claimed "all lanes" while summing only the four
...
seeds put every such module straight back. Four Plant Detail tests factory-mock
`@/hooks/useLogAiDoctorReadinessToDiary`; their components import it; the walk re-added it.

The graph is now walked **per test file**, with that test's replaced modules blocked at every depth,
and the results unioned — so a module stays reached when _any_ test loads it for real, which is the
question the figure answers. 554 of the 2,908 test files replace something; the rest share one
unblocked walk, which is exactly equivalent because with no blocking the closure of a union equals
the union of the closures. Verified against a naive walk-every-test-separately implementation: both
give 1,637 / 423 / 46.

Effect: reached **2,061 → 2,060**, unreached **45 → 46**, reachability **97.9% → 97.8%**, and
...
**28 is the only defect in this table that moved the §0 headline**, and it moved it down: 15.2% →
**12.9%**. `productSet` was built from `src/` alone, so 57 tests that import and execute modules
under `scripts/` or `supabase/functions/` — `apply-*-forward-repair.test.ts`,
`ai-credit-service-contract-effect.test.ts` and the rest — registered as importing nothing and were
filed scan-only. An edge function is product code that happens not to live in `src/`. Classification
now uses an **executable set** that includes those roots; reachability keeps its `src/`-only
denominator, because "which `src/` module does a test load" is a different question from "does this
test run repository code at all", and conflating them would have moved 97.8% for the wrong reason.

Both directions matter here. The headline **overstated** the substring-assertion problem by 2.3
points — and the bucket that remains is **more** concentrated in substring checks, 74.9% → **85.0%**.
The qualitative finding in §3 survives defect 28 intact; the number attached to it did not. A
knock-on worth naming: the literal-array caveat measured **4 of 14,054** when the bucket was wider
and is now **1 of 11,253**, because three of those four files execute a module outside `src/`.

27 is the third counting defect of the same family as 10 — a real registration the parser could not
see. `(HAS_ACTION_QUEUE_TABLE ? it : it.skip)(…)` and a `const maybeIt = pwsh ? it : it.skip` alias
both register a case; only _which_ registrar runs is conditional. Their skip state is deliberately
**not** counted, because it is decided at runtime and `.skip` means a statically declared skip — so
that figure stays 11 and keeps its meaning.

29 is not a miscount but a **scope claim that outran the tool**. §0 said "the whole test estate" for
a measurement that never parsed `node:test`. 21 such files live under `scripts/`, 19 referenced by a
workflow or a `package.json` script. Collecting that lane means parsing its registration, resolving
its runner invocations and re-deriving the never-executed analysis across it — a slice, not a
correction. The scope claim was narrowed instead, and the lane is now stated as `NOT_MEASURED` where
a reader will see it rather than left silently outside the number.

**30 is the second time a fix of mine left its own figure stale**, and it is the same shape as 24:
defect 27 moved the case total and updated §3.1 and the headline, but §1's reproducibility note and
limit 4 each carried 31,941 two sections away. A reader running the advertised command would have
got 31,965 and found the page around it disagreeing — the one failure mode §1 exists to prevent.
Both live references now read 31,965; the two in this table stay at 31,941 because they are
historical before/after records, not claims about the current tree. That distinction is why a blind
find-and-replace would have been the wrong fix.

Twice now the defect has been _the commit that fixed the previous defect not finishing the job_. The
