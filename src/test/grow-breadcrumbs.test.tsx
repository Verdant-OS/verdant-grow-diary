...
describe("GrowBreadcrumbs — component", () => {
  it("renders grow name when provided", () => {
    renderWithRouter(<GrowBreadcrumbs growId="grow-1" growName="Blue Dream" current="Plants" />);
    expect(screen.getByText("Grows").closest("a")).toHaveAttribute("href", "/grows");
    expect(screen.getByText("Blue Dream").closest("a")).toHaveAttribute("href", "/grows/grow-1");
    expect(screen.getByText("Plants")).toBeInTheDocument();
  });

  it("falls back to 'This Grow' when growId is set but growName is missing", () => {
    renderWithRouter(<GrowBreadcrumbs growId="grow-x" current="Logs" />);
  ...
  it("inserts an Actions crumb when actionId is provided", () => {
    renderWithRouter(
      <GrowBreadcrumbs growId="g1" growName="Blue Dream" current="Action Detail" actionId="a1" />,
    );
    expect(screen.getByText("Actions").closest("a")).toHaveAttribute("href", "/actions?growId=g1");
...
describe("Page wiring — GrowBreadcrumbs usage", () => {
  it.each(Object.entries(PAGES))("%s imports and renders <GrowBreadcrumbs />", (_name, src) => {
  ...
  it("ActionDetail passes actionId so the Actions crumb is inserted", () => {
    expect(PAGES.ActionDetail).toMatch(/<GrowBreadcrumbs[\s\S]*?actionId=\{row\.id\}[\s\S]*?\/>/);
    expect(PAGES.ActionDetail).toMatch(/<GrowBreadcrumbs[\s\S]*?current=\s*["']Action Detail["']/);
  });
  ...
  describe("GrowBreadcrumbs — grow switcher", () => {
    it("appears when multiple grows are available and section is provided", () => {
      renderWithRouter(
        <GrowBreadcrumbs growId="g1" growName="Blue Dream" current="Plants" section="plants" />,
      );
    ...
    it("does not appear when section is omitted", () => {
      renderWithRouter(<GrowBreadcrumbs growId="g1" growName="Blue Dream" current="Plants" />);
    ...
    function renderWithProbe(
      section: Parameters<typeof buildSwitcherTarget>[0],
      opts?: { growId?: string; actionId?: string },
    ) {
      return render(
    ...
    it("buildSwitcherTarget produces expected routes", () => {
      expect(buildSwitcherTarget("logs", "g2")).toBe("/timeline?growId=g2");
    ...
    it("pages pass the correct section prop", () => {
      ...
      expect(PAGES.Timeline).toMatch(
        /section=\{isLogsRoute\s*\?\s*["']logs["']\s*:\s*["']timeline["']\}/,
      );
    });
  });
});
