...
**Verified from source at:** `4ddb23225918ee1fdfb238ab84708a2e765adb43` (deploy branch
...
so none has grown.
The pattern was never recorded. At `4ddb2322` it was reconstructed as a quoted pair of the six
labels either side of one `|`, over `.ts`, `.tsx`, `.js` and `.mjs` files in `src/`, excluding any
`test/` or `__tests__/` directory and `*.test.*` / `*.spec.*` files. That reconstruction reproduces
the second and third figures exactly and gives 75 lines in 66 files for the first, identically at
`9b06be3f`, `5370782` and `4ddb2322`: the one-line gap is the unrecorded pattern, not drift. The
recorded figures stand:
