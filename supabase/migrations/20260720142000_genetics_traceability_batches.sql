...
-- Propagation batches (operational batches) + append-only status history +
-- shared lineage advisory-lock helper + batch upsert RPC.
--
-- Design notes:
--   * Counts are plain nullable ints (missing != 0) with an explicit
--     counts_unknown flag; batch dates are nullable (missing != now()).
--   * batch_code is owner-unique via a NAMED constraint; the RPC pre-checks it
--     so a duplicate is a clean 'batch_code_exists', never a laundered replay.
--   * genetics_lock_lineage(uid) is the ONE advisory-lock helper both
--     lineage-mutating RPCs (batch_upsert here, assign_plants in Slice 3) call
--     first, so concurrent mother/assignment edges cannot jointly form a cycle.

BEGIN;

-- ---------------------------------------------------------------------------
-- Shared per-owner lineage advisory lock (Slice 3's cycle trigger cannot see a
-- concurrent transaction's uncommitted edge under READ COMMITTED; this lock is
-- what actually closes the concurrent-cycle window).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- propagation_batch_status_events (append-only history)
-- ---------------------------------------------------------------------------
...
-- APPEND-ONLY: authenticated may read; rows are written only by the definer RPC.
-- No UPDATE/DELETE grant or policy — RLS default-denies both.
...
-- ---------------------------------------------------------------------------
-- RPC: genetics_batch_upsert(p_idempotency_key, p_payload)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Serialize this owner's lineage writes (mother edge) against concurrent
  -- assignment writes so the two cannot jointly form a cycle.
  ...
  -- Ownership re-checks (identical reason for absent vs foreign — no oracle).
  ...
  -- Pre-check batch_code uniqueness so a duplicate is a clean typed error, not a
  -- unique_violation laundered through the idempotency handler.
  IF EXISTS (
