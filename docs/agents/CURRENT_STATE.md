...
**`NO_MATCH`** — the served tree hash reproduces from no scanned commit and is
not that commit's own tree. Commit _identity_ is `PASS`; commit _provenance_ is
not. The earlier same-day stamp `92a983b4832e` (#1061) measured `dirty: false`
...
| Production commit                           | Identity `PASS`, provenance **`NO_MATCH`** — re-measured 2026-08-21T12:57 UTC. Production serves real SHA `1400a7e77eff` (`commitSource: "git"`), but the stamp is **`dirty: true`** with `ref: "__orphan__"` and `tag: null`, and `scripts/resolve-release-provenance.mjs --hash=b797bd1c7482… --ref=1400a7e77eff --scan=8` returned **NO_MATCH**: no scanned commit reproduces the served `treeHash`. Confirmed directly — the served tree hash `b797bd1c7482…` is **not** `1400a7e77eff`'s own tree (`9d92c01e0565…`), which is what `dirty: true` predicts. Read this as an editor-modified or pipeline-mutated snapshot, per the resolver's own guidance; it is **not** a clean, recomputable release. Do **not** record this as a provenance `PASS`. Supersedes the earlier same-day stamp `92a983b4832e` (#1061), which measured `dirty: false` and MATCH by recomputation — so the dirty/NO_MATCH state is **new as of the 12:11Z publish**, not a standing condition. Publish lags git — see the branch topology row. Single observations remain point-in-time |
