...
describe("aiDoctorFollowUpRules — eligibility", () => {
  ...
  it("requires grow/plant/tent context", () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.ok === false && r.reason).toBe("missing_grow_or_plant_or_tent_context");
  });
  it("is eligible with diagnosis + plant", () => {
    expect(evaluateFollowUpEligibility({ diagnosisSummary: "x", plantId: "p" }).ok).toBe(true);
  });
});
...
describe("aiDoctorFollowUpRules — draft builder", () => {
  ...
  it("builds draft from weak_context diagnosis with guardrail", () => {
    const d = buildAiDoctorFollowUpDraft(baseInputs({ alignment: weakAlignment() }));
    expect(d.posture).toBe("weak_context");
    expect(
      d.guardrails.some((g) =>
        /Do not make aggressive nutrient, irrigation, or equipment changes/i.test(g),
      ),
    ).toBe(true);
  ...
  it("includes missing Environment Check metrics in checklist", () => {
    const d = buildAiDoctorFollowUpDraft(
      baseInputs({
        moreDataNeededLabels: ["Air temperature (temp_f)", "Soil moisture (soil_moisture_pct)"],
      }),
    );
    expect(
      d.checklist.some((c) =>
        /Capture missing Environment Check metrics.*temp_f.*soil_moisture_pct/i.test(c),
      ),
    ).toBe(true);
  });
  ...
  it("isDuplicateFollowUp detects existing key", () => {
    const d = buildAiDoctorFollowUpDraft(baseInputs());
    expect(isDuplicateFollowUp(d.idempotencyKey, [d.idempotencyKey])).toBe(true);
...
describe("AiDoctorFollowUpCheckButton — rendering", () => {
  it("renders disabled copy when required context missing (no diagnosis)", () => {
    render(<AiDoctorFollowUpCheckButton inputs={{ plantId: "p" } as AiDoctorFollowUpInputs} />);
    expect(screen.getByTestId("ai-doctor-follow-up-disabled")).toHaveAttribute(
      "data-reason",
      "missing_diagnosis",
    );
  ...
  it("renders button when context exists", () => {
    ...
    expect(screen.getByTestId("ai-doctor-follow-up-button")).toHaveTextContent(
      /Create 24-hour Follow-Up Check/i,
    );
  });
  ...
  it("opens preview dialog before create", () => {
    ...
    expect(screen.getByTestId("ai-doctor-follow-up-title")).toHaveTextContent(/Northern Lights/);
  ...
  it("falls back to draft-only mode when no onCreate is provided", async () => {
    render(<AiDoctorFollowUpCheckButton inputs={baseInputs()} />);
    fireEvent.click(screen.getByTestId("ai-doctor-follow-up-button"));
    expect(screen.getByTestId("ai-doctor-follow-up-draft-only")).toBeInTheDocument();
    expect(screen.getByTestId("ai-doctor-follow-up-create")).toHaveTextContent(/Mark created/);
  });

  it("calls onCreate when provided and shows already-created state after", async () => {
    const onCreate = vi.fn().mockResolvedValue(undefined);
    render(<AiDoctorFollowUpCheckButton inputs={baseInputs()} onCreate={onCreate} />);
    fireEvent.click(screen.getByTestId("ai-doctor-follow-up-button"));
    ...
    await waitFor(() =>
      expect(screen.getByTestId("ai-doctor-follow-up-already-created")).toBeInTheDocument(),
    );
  });
  ...
  it("renders guardrails for weak_context diagnosis in the dialog", () => {
    render(<AiDoctorFollowUpCheckButton inputs={baseInputs({ alignment: weakAlignment() })} />);
    fireEvent.click(screen.getByTestId("ai-doctor-follow-up-button"));
    expect(screen.getByTestId("ai-doctor-follow-up-guardrails").textContent).toMatch(
      /Do not make aggressive nutrient, irrigation, or equipment changes/i,
    );
    expect(screen.getByTestId("ai-doctor-follow-up-posture")).toHaveTextContent(/Weak context/);
  });
