 ...
 */
export default function LeadDetailSnapshotCard({ lead }: LeadDetailSnapshotCardProps) {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="text-xs text-muted-foreground">Created {s.createdLabel}</div>
        </div>
      ...
      <div className="flex flex-wrap gap-1 text-xs">
        <Badge variant={s.statusKnown ? "secondary" : "outline"}>{s.status}</Badge>
        <Badge variant={s.leadTypeKnown ? "outline" : "destructive"}>{s.leadType}</Badge>
        <Badge variant={s.sourceKnown ? "outline" : "destructive"}>{s.source}</Badge>
      </div>
      ...
      <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
        ...
        <dd className="font-medium text-foreground">
          {s.nextAction.label}
          <span className="ml-1 text-muted-foreground">({s.nextAction.priority})</span>
        </dd>
      ...
      {s.warnings.length > 0 && (
        <p className="text-xs text-destructive" data-testid="lead-detail-snapshot-warnings">
          {s.warnings.join("; ")}
        </p>
      )}
    </div>
