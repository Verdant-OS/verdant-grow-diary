-- Narrow entitlement gates to the canonical Lovable Paddle lane (subscriptions).
--
-- Decision (2026-07-16): the Lovable lane (payments-webhook → public.subscriptions)
-- is the canonical entitlement source of truth. The legacy BYO lane
-- (paddle-webhook → public.billing_subscriptions) becomes an operator-audit
-- surface only. Both SECURITY DEFINER entitlement RPCs are narrowed to read
-- from public.subscriptions only. billing_subscriptions itself is unchanged.
--
-- Safety net: any currently-entitling billing_subscriptions row is first
-- backfilled into public.subscriptions as an environment='live' row using a
-- deterministic synthetic paddle_subscription_id so no live entitlement is
-- lost across the transition. Backfill is idempotent (ON CONFLICT DO NOTHING).
-- Founder Lifetime rows honor the subscriptions_lifetime_prefix_check
-- CHECK constraint by prefixing the synthetic id with 'lifetime_'.
...
SELECT
  bs.user_id,
  CASE
    WHEN bs.plan_id = 'founder_lifetime'
      THEN 'lifetime_byo_backfill_' || bs.id::text
    ELSE 'byo_backfill_' || bs.id::text
  END                                                             AS paddle_subscription_id,
  COALESCE(bs.provider_customer_id, 'byo_backfill_' || bs.id::text) AS paddle_customer_id,
  bs.plan_id                                                      AS product_id,
  bs.plan_id                                                      AS price_id,
  bs.status,
