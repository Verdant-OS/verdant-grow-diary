...
describe("PhenoHuntNew empty state", () => {
  ...
  it("shows empty state CTA when the grow has no plants", async () => {
    ...
    expect(anchor.getAttribute("href")).toBe("/plants?growId=g1");
    expect(screen.getByTestId("ph-empty").textContent).toMatch(
      /No candidate plants for this grow yet/i,
    );
    expect(screen.getByTestId("ph-empty-start-room")).toBeInTheDocument();
    expect(screen.getByTestId("ph-empty-lineage")).toBeInTheDocument();
  });
