...
const SensorProvenanceLegend = forwardRef<HTMLElement, SensorProvenanceLegendProps>(
  function SensorProvenanceLegend(
    { id, headingId, className, testId = "post-grow-sensor-provenance-legend" },
    ref,
  ) {
    ...
    return (
      ...
      >
        <h3 id={resolvedHeadingId} className="font-display font-semibold text-sm">
          {POST_GROW_SENSOR_PROVENANCE_LEGEND_TITLE}
        </h3>
        <ul className="mt-3 space-y-3 list-none p-0" data-testid={`${testId}-rows`}>
          {POST_GROW_SENSOR_PROVENANCE_LEGEND.map((row) => {
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1 break-words">{row.description}</span>
              </li>
            );
          })}
        </ul>
        <p className="mt-3 text-[11px] text-muted-foreground" data-testid={`${testId}-review-note`}>
          {POST_GROW_SENSOR_PROVENANCE_REVIEW_NOTE}
        </p>
      </section>
    );
