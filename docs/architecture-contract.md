...
| 2026-09-24 | `32820526d6e71c5a2ed213da35f3a68f66f86432` | Claude | Editorial follow-up to #1649; stamp unchanged. AC-1.5 no longer writes the installed Start package as a `path:line` token: it now names line 238 of `createStartHandler.js` in prose, with its `[defaultCsrfMiddleware]` fallback, re-read in `@tanstack/start-server-core` 1.169.17, the version `bun.lock` pins at this SHA. T1 covers repository paths, and a dependency file cannot be opened at the stamped SHA, so the reference is no longer written as a citation. Copilot, CodeRabbit and the Codex GitHub reviewer raised it on #1649. No other clause changed.                                                                                                                                                                                                                                                                                                                                                                                                        |
