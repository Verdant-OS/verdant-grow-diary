...
describe("shouldWarnPendingActionHasClosedSourceAlert", () => {
  ...
  it("false: pending action + open/acknowledged source alert", () => {
    expect(shouldWarnPendingActionHasClosedSourceAlert("pending_approval", "open")).toBe(false);
    expect(shouldWarnPendingActionHasClosedSourceAlert("pending_approval", "acknowledged")).toBe(
      false,
    );
  });

  it("false: non-pending action statuses + closed source alert", () => {
    for (const s of ["approved", "rejected", "completed", "canceled", "simulated"]) {
      expect(shouldWarnPendingActionHasClosedSourceAlert(s, "resolved")).toBe(false);
      expect(shouldWarnPendingActionHasClosedSourceAlert(s, "dismissed")).toBe(false);
    }
  });

  it("null-safe and deterministic", () => {
    expect(shouldWarnPendingActionHasClosedSourceAlert(null, null)).toBe(false);
    expect(shouldWarnPendingActionHasClosedSourceAlert(undefined, "resolved")).toBe(false);
    expect(shouldWarnPendingActionHasClosedSourceAlert("pending_approval", null)).toBe(false);
    expect(shouldWarnPendingActionHasClosedSourceAlert("pending_approval", "RESOLVED")).toBe(false); // case-sensitive
    const a = shouldWarnPendingActionHasClosedSourceAlert("pending_approval", "resolved");
    const b = shouldWarnPendingActionHasClosedSourceAlert("pending_approval", "resolved");
    expect(a).toBe(b);
  });
});
...
describe("ActionDetail — stale source-alert warning", () => {
  it("imports and uses the shared helper (no inline closed-status check in JSX)", () => {
    ...
    expect(ACTION_DETAIL).toMatch(/from "@\/lib\/actionQueueProvenanceRules"/);
    // The JSX should not inline the closed-status comparison itself.
    expect(ACTION_DETAIL).not.toMatch(/sourceAlertStatus\s*===\s*["']resolved["']/);
    expect(ACTION_DETAIL).not.toMatch(/sourceAlertStatus\s*===\s*["']dismissed["']/);
  });
  ...
  it("fetches the source alert only when an alert id is parseable from an alert-derived action", () => {
    ...
    expect(ACTION_DETAIL).toMatch(/const sourceAlertId = extractSourceAlertId\(row\.reason\);/);
    expect(ACTION_DETAIL).toMatch(/if \(!sourceAlertId\) return;/);
    expect(ACTION_DETAIL).toMatch(/\.from\("alerts"\)[\s\S]{0,200}\.eq\("id",\s*sourceAlertId\)/);
  });
...
>>>>>>> origin/main
  ...
  it("introduces no automation, device-control, or service_role surface", () => {
    ...
    expect(ACTION_DETAIL).not.toMatch(/auto[_-]?cancel|auto[_-]?reject|auto[_-]?approve/i);
  });
});
