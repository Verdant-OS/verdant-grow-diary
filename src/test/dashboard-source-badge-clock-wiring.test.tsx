...
describe("Dashboard source badges use capture age and the ticking clock", () => {
  ...
  it.each([
    ...
    ["manual", 25 * 60, "stale", "Manual"],
    ["csv", 0, "needs_review", "CSV"],
  ])("shows both %s badges aged %d minutes as %s", (source, age, status, label) => {
    setReading(source as string, age as number);
    renderDashboard();
    for (const testId of [
      "dashboard-tent-chart-source-badge",
      "dashboard-env-strip-source-badge",
    ]) {
      const badge = screen.getByTestId(testId);
      expect(badge).toHaveAttribute("data-status", status as string);
      expect(badge).toHaveTextContent(label as string);
      expect(badge).toHaveAttribute("data-severity", status === "usable" ? "ok" : "warning");
    }
  });

  it("changes both mounted badges from usable to stale without a new read", () => {
    setReading("live", 15);
    const originalRows = H.aggregateRows;
    const view = renderDashboard();
    for (const testId of [
      "dashboard-tent-chart-source-badge",
      "dashboard-env-strip-source-badge",
    ]) {
      expect(screen.getByTestId(testId)).toHaveAttribute("data-status", "usable");
    }
    H.now += 60_000;
    view.rerenderDashboard();
    expect(H.aggregateRows).toBe(originalRows);
    for (const testId of [
      "dashboard-tent-chart-source-badge",
      "dashboard-env-strip-source-badge",
    ]) {
      expect(screen.getByTestId(testId)).toHaveAttribute("data-status", "stale");
    }
  });
});
