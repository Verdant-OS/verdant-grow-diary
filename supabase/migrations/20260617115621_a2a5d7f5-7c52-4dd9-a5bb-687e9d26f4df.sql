-- Replace partial unique index with non-partial equivalent so PostgREST
-- upsert can infer the conflict target from the column list alone.
...
-- Extend validate_sensor_reading allow-list to include canonical V0 source
-- labels while preserving all previously accepted labels.
...
BEGIN
  ...
  IF NEW.source NOT IN (
    ...
    -- Back-compat labels preserved for existing ingest paths
    'pi_bridge','sim',
