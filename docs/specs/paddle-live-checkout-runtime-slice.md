...
- **`:244`**, inside the `catch` block, for **either** `PaddleCheckoutUnavailableError`
  **or** `PaddleCheckoutCatalogUnavailableError`: `setBlockedReason(err.message)`. Only the
  second is SKU-scoped; the first is global — its own telemetry line two lines above maps
  it to the reason token `"checkout_env_unavailable"`, not a SKU-specific one. Both error
  types funnel through this one call site and one state variable, so nothing downstream can
  tell them apart.
...
So `blockedReason` is genuinely SKU-scoped in exactly one of the three cases that set it.
...
`blockedScope: "environment" | "sku" | null` cannot be inferred in `Pricing.tsx` from which
state variable happens to be non-null — it must be set in `usePaddleCheckout.ts`, alongside
`blockedReason`, at the same three call sites. **And within the
`PaddleCheckoutCatalogUnavailableError` branch, the scope derives from the error's
`reason`, not from the class — corrected in revision 14 (Codex P2).** The class itself is
not SKU-scoped: `get-paddle-price` returns `price_resolution_unavailable` for shared
failures — missing Supabase configuration (500) and gateway/API-key outages (503) — and
`getPaddlePriceId()` wraps those in the same class as the plan-specific reasons. So:
`"sku"` only for reasons proven plan-specific (`unknown_plan`, `price_not_configured`,
`plan_sold_out`, `pack_requires_monthly_plan`); `price_resolution_unavailable` **and any
unrecognized reason** resolve `"environment"` — the same fail-closed direction §3.3 uses,
and what the class's own doc comment already instructs ("treat unknown reasons as a
generic 'unavailable' state"). The two non-catalog call sites stay `"environment"` as
revision 11 said.
...
| `src/hooks/usePaddleCheckout.ts`                              | Gates 5 and 6 — `:124` and `:137` stop comparing to the literal `"sandbox"`. **Corrected in revision 11 (Codex P2), see §3.5.1:** also expose `blockedScope: "environment" \| "sku" \| null` alongside `blockedReason`, set at the same three `setBlockedReason` call sites (`:138`, `:244`, `:246`) — and **within** the catalog-error branch, derived from the error's `reason`, not the class (revision 14): `"sku"` only for plan-specific reasons; `price_resolution_unavailable` and any unrecognized reason → `"environment"` |
...
13. **live environment, one SKU blocked by a runtime catalog failure** → that SKU's copy
    ...
    happens to be set (§3.5.1, added in revision 5, Codex P1); **and, corrected in revision
    14 (Codex P2): a `PaddleCheckoutCatalogUnavailableError` carrying
    `price_resolution_unavailable` — a shared resolver outage, not a plan failure — must
    also resolve `blockedScope: "environment"`, as must any unrecognized reason** — prove
    RED before the fix
...
**Thirty-one findings across thirteen rounds, thirty-one correct.** Finding 31 is finding
