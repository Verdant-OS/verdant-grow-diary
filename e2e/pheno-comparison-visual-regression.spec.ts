...
test("A — public /pheno-comparison demo captures cleanly (visual reference)", async ({ page }) => {
...
test.describe("C — /pheno-hunts/:id/workspace disabled/enabled Compare action", () => {
  test.skip(!HUNT_ID, "Set E2E_PHENO_HUNT_ID to run against a real hunt");
  test("workspace renders Compare action with accessible helper text", async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 1600 });
