  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <div className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span
            ...
            data-testid="feeding-history-source"
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        ...
        <MetricChip label="Input EC" value={fmtNumber(row.ec, { suffix: "mS/cm" })} />
        {row.outputEc !== null && (
          <MetricChip label="Output EC" value={fmtNumber(row.outputEc, { suffix: "mS/cm" })} />
        )}
        {row.tds !== null && (
      ...
      {row.nutrients.length > 0 && (
        <ul className="flex flex-wrap gap-1.5 mb-2">
          {row.nutrients.map((n, i) => (
            <li
              key={`${row.id}-n-${i}`}
              className="inline-flex items-center gap-1 rounded-full border border-border/40 bg-secondary/30 px-2 py-0.5 text-[11px] text-foreground/80"
            >
    ...
    <section className={"glass rounded-2xl p-4 " + (className ?? "")} aria-label="Feeding history">
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border/50 bg-secondary/20 p-4 text-center">
          <p className="text-sm text-muted-foreground">No feeding entries yet</p>
          <p className="text-[11px] text-muted-foreground/80 mt-1">
            Log a feeding from QuickLog to see it appear here.
          </p>
        </div>
      ) : (
