...
describe("normalizeEcowittTentPayload (router)", () => {
  ...
  it("flower → Flower Tent metrics only", () => {
    const s = normalizeEcowittTentPayload(payload, "flower", {
      now: NOW,
  ...
  it("seedling → Seedling Tent metrics only", () => {
    const s = normalizeEcowittTentPayload(payload, "seedling", {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
  ...
  it("vegetation → Vegetation Tent metrics only", () => {
    const s = normalizeEcowittTentPayload(payload, "vegetation", {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
  ...
  it("unknown tent key → invalid snapshot, no throw", () => {
    const s = normalizeEcowittTentPayload(payload, "lung_room", {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
  ...
  it("deterministic for identical input", () => {
    const a = normalizeEcowittTentPayload(payload, "flower", {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
    const b = normalizeEcowittTentPayload(payload, "flower", {
      now: NOW,
      captured_at_ms: CAPTURED,
    });
