/**
 * End-to-end regression for the AI Doctor "Needs follow-up" FAILURE path.
 *
 * Sibling to ai-doctor-review-workflow-error-rollback-e2e.test.tsx — locks
 * symmetric rollback semantics for the follow-up branch:
 *
 *   detail not_reviewed → Needs follow-up → server rejects → inline error
 *     → projection rolls back to not_reviewed → history empty → index has no
 *     needs_follow_up chip → "Needs follow-up: 0 visible" → reviewStatus
 *     filter excludes → built-in Needs follow-up saved view shows 0 rows
 *     → reviewStatus=not_reviewed still includes → exactly 1 attempted insert
 *     and no other write paths.
 */
...
const SESSION_ID = "sess-fu-err-1";
const OTHER_ID = "sess-fu-err-2";
...
describe("AI Doctor follow-up error-rollback workflow — static safety scan", () => {
