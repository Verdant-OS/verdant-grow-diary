-- Genetics & Propagation Traceability V1 — Slice 4
-- Append-only pathogen screening evidence ledger + record RPC.
--
-- DELIBERATELY SEPARATE from pheno_lab_results (cannabinoid/terpene COA), whose
-- uniqueness rules and chemistry semantics are unsuitable for pathogen evidence.
--
-- Evidence honesty: every row carries subject + target + a four-value result
-- (positive/negative/inconclusive/not_tested). There is no "clean" value —
-- absence of a row for a target is untested, not negative. History is immutable;
-- a correction is a NEW row linked by supersedes_id.
...
-- ---------------------------------------------------------------------------
-- RPC: genetics_screening_record(p_idempotency_key, p_payload)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Subject ownership re-check per type. One generic reason for absent-vs-foreign.
  ...
  -- A correction must reference an owned result for the SAME subject + target.
