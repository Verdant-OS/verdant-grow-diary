-- Atomic, server-recorded evidence receipts for validated AI Doctor reviews.
--
-- The receipt is intentionally narrower than a model prompt: it holds a
-- privacy-bounded, allowlisted context summary plus a keyed exact-prompt HMAC.
-- It never stores raw prompts, raw device payloads, photos, notes, IDs, or
-- provider payloads. The new Edge flow finalizes a fresh result cache and
-- receipt together in one protected transaction. Legacy cache-only rows are
-- never backfilled from a later replay.
...
-- Version one keeps this sidecar server-only. A future owner-facing projection
-- must deliberately whitelist fields rather than exposing the evidence JSONB.
-- The service role can read for server diagnostics but writes only through the
-- atomic SECURITY DEFINER finalizer below.
...
BEGIN
  ...
  END IF;
  IF p_prompt_hmac_key_id IS NULL
     OR p_prompt_hmac_key_id !~ '^[A-Za-z0-9._:-]{1,80}$' THEN
  ...
  -- This is the same per-user boundary used by spend, refund, and result
  -- attachment. It prevents a reversal or legacy attachment racing the pair.
  ...
  -- A legacy result without a receipt must remain replayable through the old
  -- path, but this finalizer must never bind a newly supplied context to it.
  ...
  INSERT INTO public.ai_doctor_review_evidence_receipts (
    ...
    p_session_id,
    p_evidence,
    p_prompt_hmac_sha256,
    p_prompt_hmac_key_id,
    p_model_id,
    p_tool_schema_version,
