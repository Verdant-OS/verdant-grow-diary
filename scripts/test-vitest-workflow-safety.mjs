...
const realPath = fileURLToPath(
  new URL("../.github/workflows/vitest-batched-full-suite.yml", import.meta.url),
);
...
t("fails when --chunk-size is not 1 (e.g. 8)", () => {
  ...
  assert.ok(
    r.errors.some((e) => /chunk-size must be 1/.test(e)),
    "expected chunk-size error",
  );
});
...
t("fails when matrix has only 8 jobs", () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => /0\.\.15/.test(e)),
    "expected matrix coverage error",
  );
});
...
t("fails when a safety gate is removed (typecheck)", () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => /typecheck/.test(e)),
    "expected missing-gate error",
  );
});
...
t("fails when a deploy/publish command is present", () => {
  ...
  assert.equal(r.ok, false);
  assert.ok(
    r.errors.some((e) => /Forbidden command/.test(e)),
    "expected forbidden-command error",
  );
});

t("fails when NODE_OPTIONS heap cap is missing", () => {
  const bad = real.replace(
    /NODE_OPTIONS:\s*--max-old-space-size=4096/,
