...
describe("GrowRoomMode VPD stage-missing info badge", () => {
  ...
  it("renders the badge with required copy and test hook", () => {
    expect(SRC).toContain('data-testid="grow-room-vpd-stage-missing-badge"');
  ...
  it("gates the badge on a present VPD value and missing tent stage", () => {
    expect(SRC).toMatch(
      /card\.snapshot\?\.vpd\s*!=\s*null\s*&&\s*\n?\s*\(tentStageById\[card\.tentId\]\s*\?\?\s*null\)\s*===\s*null\s*&&\s*\(\s*<div[\s\S]*?grow-room-vpd-stage-missing-badge/,
    );
  });
