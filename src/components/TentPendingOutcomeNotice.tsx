/**
 * TentPendingOutcomeNotice — calm, read-only TentDetail notice for
 * completed actions scoped to this tent that still need a grower-recorded
 * follow-up outcome.
 *
...
import { buildTentPendingOutcomeNoticeViewModel } from "@/lib/tentPendingOutcomeNoticeRules";

export interface TentPendingOutcomeNoticeProps {
  readonly growId: string | null | undefined;
  readonly tentId: string | null | undefined;
}

export default function TentPendingOutcomeNotice({
  growId,
  tentId,
}: TentPendingOutcomeNoticeProps) {
  const state = useDashboardPendingOutcomeReviews(growId ?? null);

  const viewModel = useMemo(() => {
    if (state.status !== "ok") {
      return buildTentPendingOutcomeNoticeViewModel({
        tentId,
        pendingReviews: [],
      });
    }
  ...
  return (
    <section
      ...
      aria-label="Pending follow-up outcomes for this tent"
      data-testid="tent-pending-outcome-notice"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p
            ...
            data-testid="tent-pending-outcome-notice-count"
          >
            {count} completed {count === 1 ? "action is" : "actions are"} waiting for a recorded
            outcome on plants in this tent.
          </p>
        </div>
      </div>

      <ul className="mt-3 space-y-2" data-testid="tent-pending-outcome-notice-list">
        {viewModel.items.map((item) => (
          <li
            key={item.actionId}
            className="flex items-center justify-between gap-2 rounded-lg border border-border/60 px-3 py-2"
            data-testid="tent-pending-outcome-notice-item"
          >
            ...
            <Button asChild size="sm" variant="ghost">
              <Link to={item.href} data-testid="tent-pending-outcome-notice-cta">
