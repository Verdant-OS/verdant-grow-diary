...
function loadIdempotencyMigration(): string {
  for (const f of readdirSync(MIG_DIR)) {
    ...
    if (
      /CREATE\s+TABLE\s+public\.pi_ingest_idempotency_keys/i.test(txt)
    )
      return txt;
  }
  return "";
...
describe("pi_ingest_idempotency_keys — required columns", () => {
  it.each([
    ...
    ["tent_id", /\btent_id\s+uuid\s+NOT\s+NULL\s+REFERENCES\s+public\.tents\(id\)\s+ON\s+DELETE\s+CASCADE/i],
    ...
    ["sensor_reading_id", /\bsensor_reading_id\s+uuid\s+NULL\s+REFERENCES\s+public\.sensor_readings\(id\)\s+ON\s+DELETE\s+SET\s+NULL/i],
...
describe("pi_ingest_idempotency_keys — indexes", () => {
  it("indexes (user_id, tent_id, created_at DESC)", () => {
    expect(SQL).toMatch(/CREATE\s+INDEX[\s\S]*?\(\s*user_id\s*,\s*tent_id\s*,\s*created_at\s+DESC\s*\)/i);
  });
  it("indexes (user_id, bridge_id, created_at DESC)", () => {
    expect(SQL).toMatch(/CREATE\s+INDEX[\s\S]*?\(\s*user_id\s*,\s*bridge_id\s*,\s*created_at\s+DESC\s*\)/i);
  });
});

describe("pi_ingest_idempotency_keys — RLS", () => {
  it("enables RLS", () => {
    expect(SQL).toMatch(/ALTER\s+TABLE\s+public\.pi_ingest_idempotency_keys\s+ENABLE\s+ROW\s+LEVEL\s+SECURITY/i);
  });
  ...
  it("does NOT define an UPDATE policy", () => {
    expect(SQL).not.toMatch(/CREATE\s+POLICY[\s\S]*?pi_ingest_idempotency_keys[\s\S]*?FOR\s+UPDATE/i);
...
describe("pi_ingest_idempotency_keys — forbidden payload columns", () => {
  ...
  it.each(["raw_payload", "raw_body", "signature", "secret", "value", "hmac"])(
    ...
    (col) => {
      ...
      const body = SQL.match(/CREATE\s+TABLE\s+public\.pi_ingest_idempotency_keys\s*\(([\s\S]*?)\);/i)?.[1] ?? "";
...
describe("pi-ingest idempotency — repo-level safety", () => {
  it("does not add idempotency_key column to sensor_readings", () => {
    for (const f of readdirSync(MIG_DIR)) {
      const txt = readFileSync(join(MIG_DIR, f), "utf8");
      expect(txt).not.toMatch(/ALTER\s+TABLE\s+public\.sensor_readings[\s\S]*?ADD\s+COLUMN[\s\S]*?idempotency_key/i);
    }
  });
  ...
  it("no src/lib pi-ingest module references service_role", () => {
    const files = walk(resolve(ROOT, "src/lib")).filter((p) => /piIngest/i.test(p));
