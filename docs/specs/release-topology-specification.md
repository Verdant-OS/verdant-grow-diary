...
- **D-RT-12 — Serving is resolved per hostname, and a split is a finding.** Every production
  ...
  and the project alias — is resolved to a deployment (M10). Other projects' domains in the team
...
- **D-RT-13 — Every change to what production serves is a publish action.** A promote, redeploy,
  ...
  auto-assignment) needs the same authority as Publish.
...
| **Why the production hostnames stopped following deploy-branch builds after the 09:45 Instant Rollback**                                         | `NOT_MEASURED` | `inference` so far: an Instant Rollback pauses auto-assignment until an explicit promote (§5.7). Closed when the owner reads the project's production-domain auto-assignment state, or promotes the tip build and watches whether the next merge follows on its own (M10 before and after) |
