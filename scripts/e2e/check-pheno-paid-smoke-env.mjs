...
const SESSION_FILE_ENVS = [
  ...
  "E2E_PHENO_PRO_ANNUAL_SESSION_FILE",
  "E2E_PHENO_CRAFT_SESSION_FILE",
  "E2E_PHENO_CRAFT_ANNUAL_SESSION_FILE",
...
const CREDENTIAL_ENV_PAIRS = [
  ...
  ["Pro Annual user", "E2E_PHENO_PRO_ANNUAL_EMAIL", "E2E_PHENO_PRO_ANNUAL_PASSWORD"],
  ["Craft user", "E2E_PHENO_CRAFT_EMAIL", "E2E_PHENO_CRAFT_PASSWORD"],
  ["Craft Annual user", "E2E_PHENO_CRAFT_ANNUAL_EMAIL", "E2E_PHENO_CRAFT_ANNUAL_PASSWORD"],
...
const FIXTURE_ENVS = [
  ...
  [
    ...
    { optional: true, note: "signal not persisted — engine treats as satisfied" },
  ],
];
...
for (const [label, envName, opts] of FIXTURE_ENVS) {
  ...
  if (has) {
    anyPresentFlag = true;
    lines.push(`  PRESENT  ${label} (${envName})`);
    continue;
  }
  if (opts.note) {
...
if (!anyPresentFlag) {
  ...
  lines.push(
    "Result: PRESENT — smoke will run whatever scenarios have inputs; others skip cleanly.",
  );
}
console.log(lines.join("\n"));
process.exit(0);
