...
function dispatchQuickLog(detail: Record<string, unknown> = {}) {
      ...
      {isLoading ? (
        ...
        >
          {Array.from({ length: 2 }).map((_, i) => (
            <li key={i} className="h-12 rounded-lg bg-secondary/40 animate-pulse" aria-hidden />
          ))}
        ...
        <ul data-testid="plant-detail-whats-missing-list" className="space-y-2">
          {prompts.map((p) => (
            <li
              key={p.kind}
                ...
                <div className="shrink-0">
                  {p.cta.event === "open-quicklog" ? (
                    <Button
                      ...
                      variant="outline"
                      className="h-7 gap-1"
                      onClick={() => dispatchQuickLog(p.cta?.eventPayload)}
