...
promotion axis re-measured for Vercel Rolling Releases by Vercel reads at 15:22–15:40 UTC
(Appendix C). That amendment adds no repository `path:line` cite.
...
- **Since 01:27 UTC on 2026-09-26, promotion is a staged rollout that someone starts.** Vercel
  Rolling Releases was enabled for the project then. It is configured as 10 % for five minutes, then
  100 %, advancing automatically. Of the nine deploy-branch production builds created after that
  (one `ERROR`, eight `READY`), exactly two were rolled out, and each rollout began with an explicit
  start event. The other six were neither a rollout's canary nor queued for one; the latest is the
  tip `d510eb56` (Appendix C). At 15:30 UTC every production hostname served the `408c966c` build,
  so the release state is **`FAIL`** under D-RT-12. The files that differ between it and the tip are
...
`established fact`, GitHub API, `git` and Vercel reads, 2026-09-26 between 15:20 and 15:40 UTC.
...
- **What changes.** The header, §1, §3, §4, §5.7, D-RT-12, D-RT-13, M10, M11, AT-10, AT-14,
  AT-15, §10, §12, §14, §15, the closing verdict, and a new Appendix C. Appendices A and B are
  not edited (D-RT-11).
...
picked up. Appendix C observed (F) become a staged rollout that someone starts: two of eight READY
builds were rolled out, and six were never picked up.
...
- **From 01:27 UTC on 2026-09-26 the path in use is path 5** (Appendix C). The measured facts are:
  ...
  - **In this project, no deploy-branch build reached the rollout without an explicit start**,
    which is `established fact` over that window. Whether that is the project's setting or a
    consequence of the 2026-09-25 Instant Rollback is `NOT_MEASURED` (§10).
...
| `docs/preview-deployment-verification.md:3-8, 14-27`            | preview-only Vercel project `verdant-command-center-preview`, npm, `/index.html` rewrite                                   | `NOT_MEASURED` | Stale: it predates SSR, and the project's existence and settings were not read (§10). Retirement candidate (§14); not edited here                                                                             |
...
| AT-15 | Rolling Releases is present end to end: path 5 in §5.7 and the §3 diagram, D-RT-12's in-flight clause, D-RT-13's rolling-release actions, M10's rollout-first read, M11's `project-rolling-release-*` events, and the §10 rows it opens                                                                                                                                                                                                        |
...
| **Whether a deploy-branch production build starts a rolling release on its own in this project**                                                 | `NOT_MEASURED` as a setting | Observed: none of the eight `READY` builds after 01:27 UTC on 2026-09-26 started one (Appendix C). Vercel's documentation is ambiguous (§5.7). Closed when the owner reads the project's rollout-trigger setting, or when M10 before and after the next merge shows a rollout that no start event preceded                                                                |
...
verified_by:
  - Vercel reads 15:22–15:40 UTC (Appendix C): get_rolling_release, get_rolling_release_config,
...
time. Since 01:27 UTC on 2026-09-26 it is a staged rollout that someone starts. Two of eight
...
change). Closing it is a rollout the owner starts, and this document does not start it.
