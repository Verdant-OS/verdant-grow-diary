-- Genetics & Propagation Traceability V1 — Slice 6
-- Bidirectional trace resolver + honest per-subject evidence rollup.
--
-- Unifies the NEW production lineage (accession -> batch -> plant) with the
-- EXISTING pheno keeper/clone/cross records, adapted READ-ONLY. Every edge
-- filters user_id on BOTH endpoints; the seed is ownership-gated. Node identity
-- is (kind, id); traversal is path-guarded, depth-capped, and statement_timeout-
-- bounded; DISTINCT ON (kind, id) picks a deterministic min-depth representative.
-- `truncated` is truthful for both the node cap and the depth frontier.

BEGIN;

-- ---------------------------------------------------------------------------
-- genetics_subject_evidence(owner, subject_type, subject_id) -> jsonb
--   Per-target CURRENT (non-superseded, latest) screening posture + a worst-wins
--   rollup that NEVER renders untested/inconclusive/superseded as negative or
--   clean. There is no 'clean'/'healthy'/'pathogen_free' state — absence is
--   'untested'.
-- ---------------------------------------------------------------------------
...
AS $function$
  ...
  latest AS (
    SELECT target, result, collected_date FROM cur WHERE rn = 1
  ),
  agg AS (
    SELECT
...
-- ---------------------------------------------------------------------------
-- genetics_trace_resolve(subject_type, subject_id, direction, max_depth, max_nodes)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Seed ownership gate (identical reason for absent vs foreign — no oracle).
  ...
  -- Backstop against pathological graphs (truthfully reported via truncated on
  -- the client side if the walk is cut short by depth/node caps first).
  PERFORM set_config('statement_timeout', '5000', true);

  WITH RECURSIVE
  -- child -> parent (ancestor direction). BOTH endpoints owner-owned.
  edges AS (
  ...
  -- Directional walk edges (src -> dst in traversal order).
  walk AS (
  ...
  ranked AS (
    SELECT DISTINCT ON (kind, id)
      kind, id, depth, edge_type, from_kind, from_id
    FROM trav
    ORDER BY kind, id, depth ASC, edge_type NULLS FIRST
  ),
  node_total AS (SELECT count(*) AS c FROM ranked),
  capped AS (
    SELECT * FROM ranked ORDER BY depth, kind, id LIMIT v_cap
  ),
  frontier AS (
    SELECT EXISTS (
