...
| Production hostname and deployed tag          | `BLOCKED_BY_ACCESS` | In the selected property, confirm no preview, Lovable, Vercel, or alternate-domain stream is being treated as production. Public source proves the deployed host and tag, not the selected GA4 stream. | Confirm the authenticated production stream matches the public host and tag.                              |
