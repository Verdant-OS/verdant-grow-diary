...
describe("sensorHistoryImportAuditLog", () => {
  ...
  it("records a Verdant Genetics XLSX import event", () => {
    ...
    expect(evt?.canonicalSourceLabel).toBe(SENSOR_HISTORY_CANONICAL_SOURCE_LABEL);
  ...
  it("records Spider Farmer and Vivosun CSV import events", () => {
    ...
    const all = readSensorHistoryImportAuditEvents(opts);
    expect(all.map((e) => e.sourceAppId)).toEqual(["spider_farmer", "vivosun"]);
  ...
  it("sanitizes input — does not store private/forbidden fields", () => {
    ...
    const stored = opts.storage!.getItem(SENSOR_HISTORY_IMPORT_AUDIT_STORAGE_KEY)!;
...
describe("SensorHistoryImportAuditLedger component", () => {
  ...
  it("renders empty-state copy when no events", () => {
    ...
    expect(screen.getByText(SENSOR_HISTORY_IMPORT_AUDIT_LEDGER_TITLE)).toBeTruthy();
    expect(screen.getByText(SENSOR_HISTORY_IMPORT_AUDIT_LEDGER_DISCLAIMER)).toBeTruthy();
    expect(screen.getByText(SENSOR_HISTORY_IMPORT_AUDIT_LEDGER_EMPTY)).toBeTruthy();
  });
  ...
  it("renders recent events with source label, CSV history, counts, range, tent/group labels", () => {
    ...
    const rows = screen.getAllByTestId("sensor-history-import-audit-ledger-row");
    expect(rows).toHaveLength(2);

    // newest-first → spider_farmer first
    expect(rows[0].getAttribute("data-source-app")).toBe("spider_farmer");
    expect(rows[1].getAttribute("data-source-app")).toBe("verdant_genetics_xlsx");

    const xlsxRow = rows[1];
    expect(within(xlsxRow).getByText(/Verdant Genetics XLSX/)).toBeTruthy();
    expect(within(xlsxRow).getByTestId("ledger-canonical-source").textContent).toBe("CSV history");
    expect(within(xlsxRow).getByTestId("ledger-file-type").textContent).toBe("XLSX");
    expect(within(xlsxRow).getByTestId("ledger-accepted-count").textContent).toMatch(
      /Accepted: 200/,
    );
    expect(within(xlsxRow).getByTestId("ledger-rejected-count").textContent).toMatch(/Rejected: 4/);
    expect(within(xlsxRow).getByTestId("ledger-date-range").textContent).toMatch(
      /2026-04-01.*2026-04-30/,
    );
    expect(within(xlsxRow).getByTestId("ledger-mapped-tents").textContent).toMatch(
      /Tent A, Tent B/,
    );
    expect(within(xlsxRow).getByTestId("ledger-mapped-groups").textContent).toMatch(/VG Group 1/);
  });
  ...
  it("limits the rendered list to 10 events", () => {
    ...
    render(<SensorHistoryImportAuditLedger options={opts} />);
    expect(screen.getAllByTestId("sensor-history-import-audit-ledger-row")).toHaveLength(10);
  });

  it("does not render raw_payload, device serials, bridge tokens, or import batch IDs", () => {
    const opts = seed();
    const { container } = render(<SensorHistoryImportAuditLedger options={opts} />);
  ...
  it("renders inserted + skipped-duplicates counts when present and '—' when absent", () => {
    ...
    const newRow = rows[1];
    expect(within(newRow).getByTestId("ledger-inserted-count").textContent).toMatch(
      /Inserted:\s*120/,
    );
    expect(within(newRow).getByTestId("ledger-duplicate-count").textContent).toMatch(
      /Skipped duplicates:\s*2146/,
    );
    expect(within(legacyRow).getByTestId("ledger-inserted-count").textContent).toMatch(
      /Inserted:\s*—/,
    );
    expect(within(legacyRow).getByTestId("ledger-duplicate-count").textContent).toMatch(
