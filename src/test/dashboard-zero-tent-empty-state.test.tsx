...
describe("DashboardZeroTentEmptyState — render", () => {
  it("renders the headline, support copy, expectation reset, and CTA", () => {
    ...
    expect(
      screen.getByText(/Your dashboard starts with a real grow space\. Add a tent first/i),
    ).toBeTruthy();
    ...
    expect(reset).toHaveTextContent(/only data you add or connect appears here/i);
  ...
  it("does not promise the retired /demo route", () => {
    ...
    const reset = screen.getByTestId("dashboard-zero-tent-expectation-reset");
    expect(reset).not.toHaveTextContent("/demo");
    expect(reset.querySelector("a")).toBeNull();
    expect(reset.querySelector("code")).toBeNull();
  });
...
describe("Dashboard wires the zero-tent empty state", () => {
  it("imports DashboardZeroTentEmptyState", () => {
    expect(DASH_EXEC).toMatch(/from\s+["']@\/components\/DashboardZeroTentEmptyState["']/);
  });

  it("gates the env chart + environment strip on tents.length === 0", () => {
    expect(DASH_EXEC).toMatch(/tents\.length\s*===\s*0\s*\?\s*\(\s*<DashboardZeroTentEmptyState/);
  });
