...
describe("classifySnapshotFreshness", () => {
  ...
  it("classifies live older than threshold as stale and degraded", () => {
    const r = classifySnapshotFreshness(
      snap({ captured_at: "2026-06-26T10:00:00Z" }),
      { now: NOW },
    );
  ...
  it("missing or malformed captured_at is invalid", () => {
    expect(
      classifySnapshotFreshness(snap({ captured_at: null }), { now: NOW })
        .freshness,
    ).toBe("invalid");
    expect(
      classifySnapshotFreshness(snap({ captured_at: "not-a-date" }), { now: NOW })
        .freshness,
    ).toBe("invalid");
  });
  ...
  it("far-future captured_at is invalid", () => {
    const r = classifySnapshotFreshness(
      snap({ captured_at: "2030-01-01T00:00:00Z" }),
      { now: NOW },
    );
    expect(r.freshness).toBe("invalid");
    expect(r.isDegraded).toBe(true);
