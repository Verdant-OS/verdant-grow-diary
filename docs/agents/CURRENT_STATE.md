...
were added to `public/sitemap.xml` (56 → 61 `<loc>` in-repo) and `/breeder-beta`
is held as a self-declared copy-only duplicate of `/creator-beta`, with the
reasoning committed beside the exclusion in
`scripts/public-route-parity.config.mjs`.
...
| Indexable routes outside the sitemap        | `FAIL` **live**, `RESOLVED` in-repo pending publish. Measured 2026-08-20: the set is **six**, not the four previously recorded — `/glossary`, `/breeder-beta`, `/creator-beta`, `/pheno-comparison`, `/pheno-expression-showcase`, `/docs/mcp-api` — each HTTP 200, self-canonical, `index, follow`, absent from the live `sitemap.xml`, and blocked by no robots group. Cheek adjudicated SITEMAP on 2026-08-20; five are now in `public/sitemap.xml` and `/breeder-beta` is held as a near-duplicate of `/creator-beta`. **Live stays `FAIL` until the next publish** — the repo change does not move production. See blocker 8’s sibling note |
...
8. **Ahrefs structured-data findings must be triaged, not bulk-fixed.** All 56
   ...
   **`/breeder-beta` was deliberately held**: it renders the same `<BetaLanding>`
   component as `/creator-beta` — whose own source header calls the breeder route a
   "copy-only difference" — and measured 233 of ~237 shared unique visible tokens
   with identical `h1` and every `h2`. Both are self-canonical, so advertising the
   pair would set two near-identical URLs competing on the same queries. It needs a
   one-line owner call: differentiate its body copy and sitemap it, or canonicalise
   it to `/creator-beta`. The reasoning is committed alongside the exclusion in
   `scripts/public-route-parity.config.mjs`.
