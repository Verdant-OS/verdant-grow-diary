...
describe("Alerts One-Tent Loop next-step card wiring", () => {
  ...
  it("routes Review Action Queue to the /actions surface (never back to /alerts)", () => {
    renderCard(
      <OneTentLoopNextStepCard current="alert" testId="alerts-one-tent-loop-next-step-card" />,
    );
    ...
    expect(cta).toHaveTextContent(/Review Action Queue/i);
  });

  it("routes to /actions even when an alertId is present (Review Action Queue is navigation)", () => {
    renderCard(
      <OneTentLoopNextStepCard
        current="alert"
        ids={{ alertId: "a1" }}
