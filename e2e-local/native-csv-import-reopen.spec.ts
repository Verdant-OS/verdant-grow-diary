...
async function signInCurrentDocument(page: Page, f: LocalFixture) {
  ...
  // Investigation only: observe element identity and value presence, never values,
  // account identifiers, URLs, tokens, request bodies, or browser storage.
  const probe = await page.evaluateHandle(() => {
    ...
    function field(id: string) {
      ...
      if (!(element instanceof HTMLInputElement)) return { node: null, present: false };
      if (!nodes.has(element)) nodes.set(element, ++nextNode);
      return { node: nodes.get(element)!, present: element.value.length > 0 };
    }
    function record(reason: string) {
      if (events.length >= 160) return;
      const email = field("signin-email");
      const password = field("signin-password");
      events.push({
        reason,
        ms: Math.round(performance.now() - start),
        onAuthRoute: location.pathname === "/auth",
        emailNode: email.node,
        emailPresent: email.present,
    ...
    const observer = new MutationObserver(() => {
      const identity = `${field("signin-email").node}:${field("signin-password").node}`;
      if (identity !== previousNodes) {
        previousNodes = identity;
        record("field-identity-change");
      }
    });
    const onInput = (event: Event) => {
      ...
      if (
        ...
        (target.id === "signin-email" || target.id === "signin-password")
      ) {
        record(`${target.id === "signin-email" ? "email" : "password"}-${event.type}`);
      }
    };
    const onSubmit = () => record("submit");
  ...
  try {
    ...
    // Follow the displayed public entry within this same document.
    ...
    // Exactly one fill per field: no toPass loop, refill, auth retry, or reload.
    ...
    try {
      const events = await probe.evaluate((state) => state.stop());
      console.log("Native single-fill auth trace:", JSON.stringify({ phase, events }));
    } catch {
      console.log("Native single-fill auth trace: document observation unavailable.");
    }
    await probe.dispose();
  }
}
