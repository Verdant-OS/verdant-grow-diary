...
| `vercel.json` authority   | `PARTIAL / AMBIGUOUS`, "does not prove production is currently broken" | **Upheld and narrowed.** Its three measured runtime directives — `redirects`, `rewrites`, `headers` — are **not applied as declared** in production. Whole-file authority is **`NOT_MEASURED`**: `projectSettings` stays `inference`, and `cleanUrls` and `git` were never probed | §6.2                                                                             |
...
>
> **Four** publishes, **three** distinct committed trees, four mismatches, all `dirty: true`
> (an earlier revision of this sentence said three/two and went stale when the fourth row landed).
> The provenance gap is therefore **not** an artifact of the single publish this section first
...
Distinguishing all three needs the publisher's build log, which is owner-gated. A **named instance
of the second** landed on the base branch after this was written, but it is weaker than a first
reading suggests and only its confirming direction can be tested without that log — see the #1090
subsection below, and read its withdrawal before citing it. **Nothing here
...
**WITHDRAWN — the same-build version of this inference is impossible, and this document asserted
it.** An earlier revision said "every Lovable build rewrote a hashed root **before the stamper
ran**." Codex refuted it in review from `package.json:9-11`: `prebuild` runs
`verify-edge-shared-in-sync` → `check-no-src-lib-imports` → **`stamp-version.mjs`**, and only then
does `build` run `vite build`. The npm/bun lifecycle orders `pre<script>` before `<script>`, so
within one build the stamp is captured **before** any Vite hook can fire. A plugin running at
`configResolved` / `buildStart` cannot have dirtied the tree that same build's stamp already read.
This is a property of `package.json` and holds for whoever invokes the script — Codex reached it
via `vercel.json`'s `buildCommand`, but that framing is not adopted here, since §6.2 measures that
file's directives as not applied in production and production publishes through Lovable.

**What survives is a cross-build version, and it needs a premise this document cannot verify.**
The plugin was wired for `vite dev` as well as `vite build` — the guard excluded only Windows, not
the dev server. So a rewrite during an editor session, or during a _previous_ build, would sit in
the workspace when the _next_ publish's `prebuild` stamped it. That timeline is consistent with
the ordering Codex names, and it is a concrete instance of the second explanation listed above —
"a publish pipeline that mutates hashed files … before prebuild" — with a named file instead of a
hypothesis. It is **not a fourth class** of explanation, and does not narrow the list of three.

**The premise is that the build workspace persists between cycles rather than being a fresh
clone**, and it is `inference`, not measured here. What points that way: production stamps carry
`dirty: true` and `ref: "__orphan__"`, neither of which a clean checkout produces; and #1090's own
text says "every editor/publish cycle rewrote a `TREE_HASH_ROOTS` path" and asks that publishing
wait until "the workspace no longer regenerates the file" — phrasing that presumes persistence.
That is `source claim` plus `inference` stacked on the two unverified claims above. **Treat the
whole candidate as weaker than the earlier revision presented it.**

**Codex's own directive is the right one and is not in conflict with this:** investigate mutations
occurring _before_ `stamp-version.mjs`. Under the persistence premise a prior cycle's plugin
rewrite _is_ such a mutation. Under a fresh-workspace-per-build premise it is not, and the
candidate collapses entirely.
...
**The falsifiable follow-up needs no owner access — but it is one-directional, corrected after
review.** #1090 is merged but not necessarily published. Once a build produced _after_ it reaches
production, re-fetch `/version.json` and recompute that commit's tree with `computeTreeHash()`.
**If the mismatch and `dirty: true` stop, that supports the candidate.** The converse does **not**
hold: if they persist, the candidate is _not_ refuted, because a workspace already dirtied by an
earlier cycle stays dirty until something resets it — the same persistence premise that the
mechanism needs also blunts the refuting direction. An earlier revision of this paragraph claimed
both directions. A persisting mismatch after #1090 therefore returns the question to the
publisher's build log rather than settling it.
...
### 6.2 `vercel.json`'s runtime directives are measured; its whole-file authority is not
...
#### Security headers — the catch-all block measured, and the earlier claim was wrong in both directions
...
**Scope, stated up front because an earlier revision left it implicit:** all three paths hit the
catch-all `/(.*)` rule, so what follows measures **that block only**. `vercel.json` declares two
further header blocks — `/unsubscribe` (`Cache-Control`, `Referrer-Policy`, `X-Robots-Tag`) and
`/assets/(.*)` (`Cache-Control`) — which were **not probed** and are `NOT_MEASURED`.
...
| Why did the build **workspace's** hashed roots differ from that commit's tree?     | `BLOCKED` — still. #1090 (`9133a4c45`) names a candidate (a Vite plugin regenerating `supabase/functions/mcp/index.ts`, a hashed root), and the file's membership in `TREE_HASH_ROOTS` plus the wiring at `ea31fbdfb` are verified in-repo — but the same-build form of it is **impossible** (`prebuild` stamps before `vite build`; refuted in review) and the surviving cross-build form needs an unverified workspace-persistence premise (§6.1). A post-#1090 publish can **support** it; it cannot refute it. Note the question: §6.1 establishes workspace drift only. Whether any **shipped byte** differs is `NOT_MEASURED` — and the named candidate **is** a shipped edge-function source, so the never-ships examples do not cover it |
...
blocked:
  - why the build workspace's hashed roots differed from the stamped commit's tree.
    Owner: whoever holds the publisher's build log. Unblocked by that log. A post-#1090
    publish can SUPPORT the candidate mechanism #1090 names but cannot refute it, and its
    same-build form was refuted in review (§6.1) — so this stays owner-gated. Lead recorded:
...
2. **Reproduce the own-commit hash comparison**, which is the surviving finding: fetch
   ...
   ref would not touch the surviving finding. The open half is the publisher's build log; #1090
   adds a candidate whose confirming direction a post-#1090 publish can test, but read §6.1's
   withdrawal first — its same-build form was refuted in review.
