...
export const WORKFLOW_PATH = ".github/workflows/reconcile-restored-history-ledger.yml";
export const ARTIFACT_PREFIX = "restored-history-ledger-reconciliation-preflight";
...
export function validatePreflightArtifactMetadata({
  ...
  const deployHeadSha = lowercaseSha(expected?.deployHeadSha);
  const candidateHeadSha = lowercaseSha(expected?.candidateHeadSha);
  const candidatePrNumber = positiveInteger(expected?.candidatePrNumber);
  ...
  if (
    ...
    priorRun?.head_sha !== deployHeadSha ||
    currentRun?.head_sha !== deployHeadSha ||
  ...
  if (
    ...
    artifactRun?.head_sha !== deployHeadSha ||
  ...
  return Object.freeze({ artifactId, archiveDigest: digest, runAttempt });
}
...
function validateTargetMigrations(value) {
  if (!Array.isArray(value) || value.length !== TARGET_MIGRATIONS.length) return false;
  return value.every((migration, index) => {
    const expected = TARGET_MIGRATIONS[index];
    return (
      exactKeys(migration, ["name", "path", "sha256", "version"]) &&
      migration.version === expected.version &&
      migration.name === expected.name &&
      migration.path === expected.path &&
...
function validateReceipt(value, expected, runAttempt, priorRun) {
  ...
  if (
    ...
    value.tool !== "reconcile-restored-history-ledger" ||
    value.operation !== "PREFLIGHT" ||
    value.outcome !== "safe_to_reconcile" ||
    ...
    value.deploy_head_sha !== expected.deployHeadSha ||
    value.candidate_pr_number !== CANDIDATE_PR_NUMBER ||
    value.candidate_head_sha !== expected.candidateHeadSha ||
    value.project_ref !== PROJECT_REF ||
    value.manifest_sha256 !== MANIFEST_SHA256 ||
    !validateTargetMigrations(value.target_migrations) ||
...
async function main(env = process.env) {
  try {
    const expected = {
      ...
      deployHeadSha: env.EXPECTED_HEAD_SHA,
      candidatePrNumber: env.EXPECTED_CANDIDATE_PR_NUMBER,
      candidateHeadSha: env.EXPECTED_CANDIDATE_HEAD_SHA,
    ...
    console.log("Authenticated restored-history ledger PREFLIGHT artifact validated.");
    return 0;
  } catch {
    console.error(
