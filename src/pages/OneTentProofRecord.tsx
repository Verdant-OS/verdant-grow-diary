...
export default function OneTentProofRecord() {
  ...
  // Tightened source-claim chip: only render the unverified live claim when the operator both
  ...
  const showSourceChip = Boolean(sourceLabel) && (sourceLabel !== "live" || capturedAt.length > 0);
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-6">
      <header className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {showSourceChip ? (
            ...
            >
              {sourceLabel === "live"
                ? "Claimed source: live (unverified)"
                : `Source: ${sourceLabel}`}
            </span>
          ) : null}
        </div>
        <h1 className="text-2xl font-semibold tracking-tight">One-Tent Proof Record</h1>
        <p data-testid="self-report-subhead" className="text-sm font-medium text-foreground">
          Operator Self-Report (unverified)
        </p>
        <p className="text-sm text-muted-foreground">
          Operator-only export screen. Paste the captured evidence from one real or manual
          end-to-end Verdant loop. JSON is built client-side. No live data is implied unless the
          source label says so.
        </p>
        <p data-testid="allowed-labels-copy" className="text-xs text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="grow-name"
            label="Grow name"
            value={input.scope?.growName ?? ""}
            onChange={(v) => update("scope", { growName: v })}
          />
          <Field
            id="grow-id"
            label="Grow id"
            value={input.scope?.growId ?? ""}
            onChange={(v) => update("scope", { growId: v })}
          />
          <Field
            id="tent-name"
            label="Tent name"
            value={input.scope?.tentName ?? ""}
            onChange={(v) => update("scope", { tentName: v })}
          />
          <Field
            id="tent-id"
            label="Tent id"
            value={input.scope?.tentId ?? ""}
            onChange={(v) => update("scope", { tentId: v })}
          />
          <Field
            id="plant-name"
            label="Plant name"
            value={input.scope?.plantName ?? ""}
            onChange={(v) => update("scope", { plantName: v })}
          />
          <Field
            id="plant-id"
            label="Plant id"
            value={input.scope?.plantId ?? ""}
            onChange={(v) => update("scope", { plantId: v })}
          />
          <Field
            id="stage"
            label="Stage / day"
            value={input.scope?.stage ?? ""}
            onChange={(v) => update("scope", { stage: v })}
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="ql-id"
            label="Quick Log diary entry id"
            value={input.quickLog?.diaryEntryId ?? ""}
            onChange={(v) => update("quickLog", { diaryEntryId: v })}
          />
          <Field
            id="ql-action"
            label="Action type"
            value={input.quickLog?.actionType ?? ""}
            onChange={(v) => update("quickLog", { actionType: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            ...
            />
            <Label htmlFor="ql-photo" className="text-xs">
              Photo attached
            </Label>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="tl-row"
            label="Timeline row id"
            value={input.timeline?.rowId ?? ""}
            onChange={(v) => update("timeline", { rowId: v })}
          />
          <Field
            id="tl-route"
            label="Timeline route observed"
            value={input.timeline?.routeObserved ?? ""}
            onChange={(v) => update("timeline", { routeObserved: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            <input
              id="tl-chip"
            ...
            />
            <Label htmlFor="tl-chip" className="text-xs">
              Timeline chip visible
            </Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Captured values and the source label as rendered in the UI.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="r-metric"
            label="Metric"
            value={input.reading?.metric ?? ""}
            onChange={(v) => update("reading", { metric: v })}
          />
          <Field
            id="r-value"
            label="Value"
            value={String(input.reading?.value ?? "")}
            onChange={(v) => update("reading", { value: v })}
          />
          <Field
            id="r-unit"
            label="Unit"
            value={input.reading?.unit ?? ""}
            onChange={(v) => update("reading", { unit: v })}
          />
          <Field
            id="r-captured"
            label="Captured at (ISO)"
            value={input.reading?.capturedAt ?? ""}
            onChange={(v) => update("reading", { capturedAt: v })}
          />
          <div className="space-y-1">
            <Label htmlFor="r-source" className="text-xs">
              Source label (as shown)
            </Label>
            <select
              id="r-source"
              ...
              onChange={(e) =>
                update("reading", {
                  sourceLabel: (e.target.value || undefined) as ProofSourceLabel | undefined,
                })
              }
            >
          ...
          <Field
            id="r-route"
            label="Route observed"
            value={input.reading?.routeObserved ?? ""}
            onChange={(v) => update("reading", { routeObserved: v })}
          />
          <Field
            id="snapshot-route"
            label="Latest snapshot route"
            value={input.snapshotRoute ?? ""}
            onChange={(v) => setInput((p) => ({ ...p, snapshotRoute: v }))}
          />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cautious-AI session evidence (confidence, risk, presence of missing_info / do_not_do).
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="ai-session"
            label="AI Doctor session id"
            value={input.aiDoctor?.sessionId ?? ""}
            onChange={(v) => update("aiDoctor", { sessionId: v })}
          />
          <Field
            id="ai-confidence"
            label="Confidence"
            value={input.aiDoctor?.confidence ?? ""}
            onChange={(v) => update("aiDoctor", { confidence: v })}
          />
          <Field
            id="ai-risk"
            label="Risk level"
            value={input.aiDoctor?.riskLevel ?? ""}
            onChange={(v) => update("aiDoctor", { riskLevel: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            <input
              id="ai-missing"
            ...
            />
            <Label htmlFor="ai-missing" className="text-xs">
          ...
          <div className="flex items-center gap-2 pt-5">
            ...
            />
            <Label htmlFor="ai-do-not-do" className="text-xs">
              do_not_do present
            </Label>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="t-metric"
            label="Target metric"
            value={input.target?.metric ?? ""}
            onChange={(v) => update("target", { metric: v })}
          />
          <Field
            id="t-original"
            label="Original target value"
            value={String(input.target?.originalValue ?? "")}
            onChange={(v) => update("target", { originalValue: v })}
          />
          <Field
            id="t-temp"
            label="Temporary target value"
            value={String(input.target?.temporaryValue ?? "")}
            onChange={(v) => update("target", { temporaryValue: v })}
          />
          <Field
            id="t-restored-at"
            label="Restored at (ISO)"
            value={input.target?.restoredAt ?? ""}
            onChange={(v) => update("target", { restoredAt: v })}
          />
          <Field
            id="t-restore-diary"
            label="Restore diary entry id"
            value={input.target?.restoreDiaryEntryId ?? ""}
            onChange={(v) => update("target", { restoreDiaryEntryId: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            <input
              id="t-restored"
            ...
            />
            <Label htmlFor="t-restored" className="text-xs">
              Target restored to original
            </Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="a-id"
            label="Alert id"
            value={input.alert?.id ?? ""}
            onChange={(v) => update("alert", { id: v })}
          />
          <Field
            id="a-metric"
            label="Alert metric"
            value={input.alert?.metric ?? ""}
            onChange={(v) => update("alert", { metric: v })}
          />
          <Field
            id="a-sev"
            label="Alert severity"
            value={input.alert?.severity ?? ""}
            onChange={(v) => update("alert", { severity: v })}
          />
          <Field
            id="a-created"
            label="Alert created at (ISO)"
            value={input.alert?.createdAt ?? ""}
            onChange={(v) => update("alert", { createdAt: v })}
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            ...
            onChange={(v) => update("action", { id: v })}
          />
          <Field
            id="act-status"
            label="Action status"
            value={input.action?.status ?? ""}
            onChange={(v) => update("action", { status: v })}
          />
          <Field
            id="act-linked-alert"
            label="Linked alert id"
            value={input.action?.linkedAlertId ?? ""}
            onChange={(v) => update("action", { linkedAlertId: v })}
          />
          <Field
            id="act-result"
            label="Completion result"
            value={input.action?.completionResult ?? ""}
            onChange={(v) => update("action", { completionResult: v })}
          />
          <Field
            id="act-completed"
            label="Completed at (ISO)"
            value={input.action?.completedAt ?? ""}
            onChange={(v) => update("action", { completedAt: v })}
          />
          <Field
            id="act-approved-at"
            label="Approved at (ISO)"
            value={input.action?.approvalGate?.approvedAt ?? ""}
            onChange={(v) => updateApprovalGate({ approvedAt: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            <input
            ...
            />
            <Label htmlFor="act-approval-required" className="text-xs">
              Approval gate observed (required)
            </Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Field
            id="f-diary"
            label="Follow-up diary entry id"
            value={input.followup?.diaryEntryId ?? ""}
            onChange={(v) => update("followup", { diaryEntryId: v })}
          />
          <div className="flex items-center gap-2 pt-5">
            <input
              id="f-chip"
            ...
            />
            <Label htmlFor="f-chip" className="text-xs">
              Timeline follow-up chip visible
            </Label>
          </div>
          <div className="flex items-center gap-2 pt-5">
            <input
            ...
            />
            <Label htmlFor="f-link" className="text-xs">
              ActionDetail follow-up link visible
            </Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            JSON is generated entirely client-side. A printable export is deferred until the project
            ships a shared safe export helper for this surface.
          </CardDescription>
        </CardHeader>
