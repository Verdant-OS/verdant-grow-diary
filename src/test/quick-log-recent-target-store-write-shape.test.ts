 ...
 *
 * `quicklog-sensor-strip-split-guardrail` scans this module's source for
 * forbidden payload classes, which is the right tool for "this construct is
 * absent from this file". It cannot see a field that exists only at runtime,
 * and TypeScript's excess-property check only catches an object literal at the
 * call site — a widened value, a spread database row, or an `as` cast passes
 * straight through. So the static fence and this file cover different halves:
 * the scan keeps forbidden classes out of the source, and these tests keep
