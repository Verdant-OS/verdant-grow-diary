...
import {
  isSafeArtifactDeletePath,
  assertSafeArtifactDeletePath,
} from "./demo-proof-artifact-utils.mjs";
...
t("rejects repo root itself", () =>
  assert.equal(isSafeArtifactDeletePath(repoRoot, repoRoot), false),
);
t("rejects path outside repo", () =>
  assert.equal(isSafeArtifactDeletePath("/etc/passwd", repoRoot), false),
);
...
t("allows .artifacts/demo-proof-playwright-report", () =>
  assert.equal(
    isSafeArtifactDeletePath(`${repoRoot}/.artifacts/demo-proof-playwright-report`, repoRoot),
    true,
  ),
);
t("allows nested test-results trace.zip", () =>
  assert.equal(
    isSafeArtifactDeletePath(`${repoRoot}/test-results/some-spec/trace.zip`, repoRoot),
    true,
  ),
);
...
console.log("\nassertSafeArtifactDeletePath:");
t("throws on '/'", () =>
  assert.throws(() => assertSafeArtifactDeletePath("/", repoRoot), /unsafe/),
);
t("throws on repo root", () =>
  assert.throws(() => assertSafeArtifactDeletePath(repoRoot, repoRoot), /unsafe/),
);
