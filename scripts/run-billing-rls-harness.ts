...
async function main() {
  ...
  if (seedErr) {
    console.error("seed insert failed:", seedErr);
    process.exit(1);
  }
  ...
  try {
    ...
    // 1. A reads own row → exactly one row, matches uidA.
    {
      ...
      check(
        "1. A SELECT own → 1 row, matches uidA",
    ...
    // 2. A reads B's row → 0 rows (silent RLS filter).
    {
      const { data, error } = await a
        ...
        .eq("user_id", uidB);
      check(
        "2. A SELECT where user_id=B → 0 rows (RLS filters silently)",
    ...
    // 3a. A INSERT claiming own user_id → rejected.
    {
      const { data, error } = await a
        ...
        .select();
      check(
        "3a. A INSERT own user_id → rejected",
        !!error || (Array.isArray(data) && data.length === 0),
    ...
    // 3b. A INSERT claiming B's user_id (self-grant exploit) → rejected.
    {
      const { data, error } = await a
        ...
        .select();
      check(
        "3b. A INSERT arbitrary user_id → rejected",
        !!error || (Array.isArray(data) && data.length === 0),
    ...
    // 4. A UPDATE own row → rejected (no rows changed).
    {
      const { data, error } = await a
        .from("billing_subscriptions")
        .update({ plan_id: "founder_lifetime" })
        .eq("user_id", uidA)
        .select();
      const noChange = !error && Array.isArray(data) && data.length === 0;
      check(
        "4a. A UPDATE own → rejected or 0 rows affected",
        !!error || noChange,
      ...
      const { data: verify } = await admin
        ...
        .single();
      check(
        "4b. row plan_id unchanged after UPDATE attempt",
        verify?.plan_id === "free",
    ...
    // 5. A DELETE own row → rejected.
    {
      ...
      const noChange = !error && Array.isArray(data) && data.length === 0;
      check(
        "5a. A DELETE own → rejected or 0 rows affected",
        !!error || noChange,
        error?.message ?? `data=${JSON.stringify(data)}`,
      );
    ...
    // 6. anon SELECT / INSERT / UPDATE / DELETE.
    {
      ...
      check(
        "6a. anon SELECT → denied or 0 rows (no leak)",
        denied || empty,
        error?.message ?? `rows=${data?.length}`,
      );
    }
    ...
    }
    {
      const { data, error } = await anon
        ...
        .select();
      check(
        "6c. anon UPDATE → rejected or 0 rows",
        !!error || (Array.isArray(data) && data.length === 0),
        error?.message,
      );
    }
    {
      const { data, error } = await anon
        ...
        .select();
      check(
        "6d. anon DELETE → rejected or 0 rows",
        !!error || (Array.isArray(data) && data.length === 0),
        error?.message,
      );
    }
    ...
    // 7. B unaffected; both rows still 'free' (verified as service_role).
    {
      ...
      const allFree =
        Array.isArray(data) && data.length === 2 && data.every((r) => r.plan_id === "free");
      check(
        "7. final verification: both seeded rows still plan_id='free'",
        allFree,
        JSON.stringify(data),
      );
    }
