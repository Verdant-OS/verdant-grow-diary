DROP FUNCTION IF EXISTS public.quicklog_save_event(text, uuid, text, uuid, uuid, text, text, jsonb, timestamptz);
...
COMMENT ON FUNCTION public.quicklog_save_event(text, uuid, text, uuid, uuid, text, text, jsonb, timestamptz, jsonb) IS
  'Atomic Quick Log writer (v2): note-style Quick Logs MUST arrive as event_type=observation with p_details.kind=note. Trigger-aligned event-type whitelist. Sensor snapshot provenance preserved verbatim.';