...
§8.** That is not something an author can supply for themselves. **Five evidence blockers
also stand open** — C05 (single-source), C04b (conflicted), C08 (trade press only, R3),
C04a (S9 unread), C10 (S7b unread). §8.5 and the handoff carry the full list and the
distinction between them; **this count must match those two.**
...
2. **Under the ±0.5 °C / ±3% RH error budget assumed above, the plausible range is
   ...
   claim about any particular instrument — substitute your own sensors' datasheet figures.
...
- **Instrument accuracy is one of two comparable error terms, not the dominant one.** Chen's
  infrared thermometer operated over **6.0–14.0 µm** with a **post-calibration accuracy of
  0.35 °C**, against 0.15 °C for the reference thermocouple [S7a, §2.5]. The computation
  below shows emissivity error reaching or exceeding that 0.35 °C once the reflected
  temperature is 50–60 °C, so neither term dominates in general.
...
- **Using the wrong uncertainty for the question you are asking.** §3.2's worked example
  spans ~70% of the shipped band under an _assumed_ ±0.5 °C / ±3% RH budget — a property of
...
- **Lights-off transition timeline** — a recording template covering the **full lights-off
  period** on its first run: temperature, RH, dew point context, surface-temperature spot
  checks, and equipment state. **No source in §8 establishes a duration for the humidity
  peak**, and a room whose HVAC cycles slowly or whose substrate releases moisture late can
  peak after any fixed window would have stopped watching; a shorter recurring template is
  defensible only once the room's own data shows where its peak sits.
...
| S6a | Utah State University Extension, "Bud Rot" (hemp IPM note). <https://extension.usu.edu/planthealth/ipm/notes_ag/hemp-bud-rot>                                                                                                                                                                                                                                                                                                 | A (extension)                               | 2026-08-15 | **Read at source.** Supplies: cool temperatures **65–75 °F** favour spread/infection; management guidance to keep greenhouse RH **below 50%**. Page is a fact sheet without numbered sections; locator is the page itself |
| S6b | Oregon State University Extension, gray-mold-in-hemp series (incl. "Management of Gray Mold in Hemp (Botrytis Bud Blight and Stem Canker)" and "How to prevent gray mold in hemp")                                                                                                                                                                                                                                            | A (extension)                               | 2026-08-15 | **NOT READ — HTTP 403.** Cited from search-result summaries only. Apparent source of the **50–80 °F range and 68 °F optimum**; **that figure must not be published until an editor opens the page**                       |
...
| C02  | §3.4           | Averaging temperature and RH before deriving VPD produces an error whose **sign and magnitude are data-dependent**. Two terms contribute: convexity of `e°(T)`, and T–RH covariance. **Either can be the larger**; at constant RH the covariance term is exactly zero and convexity is the whole error                                                                                                                 | R1     | method/arithmetic       | Derived                                                                                               | S1 (`limits`) plus author computation                                                                                                                                          | Author-computed, reproducible                                                       |
...
| C09  | §5             | _Botrytis cinerea_ is favoured by cool temperatures (**65–75 °F**) with high RH and free water on plant surfaces; extension guidance recommends holding greenhouse RH **below 50%** and improving airflow. **All figures stated in the page come from S6a (read at source). The S6b-derived 50–80 °F range and 68 °F optimum are NOT stated anywhere in the page or assets — withdrawn pending an editor reading S6b** | R2     | pathogen risk           | Source-reported                                                                                       | S6a only (`supports`) — S6b cited in the register but **nothing is claimed from it**                                                                                           | Hemp/greenhouse **proxy setting**                                                   |
...
| C02  | Any room where T and RH both vary                                                                                                                      | kPa                                             | Convexity always biases **low**; the covariance term can add or subtract. **The net sign flips only when the covariance term exceeds the convexity term and carries the opposite sign** — with small RH variation the result stays low regardless of correlation sign | That either term dominates in general, or that the sign is predictable from the correlation sign alone                                                        |
...
| C09  | **PARTIAL — S6b unread**                   | PENDING              | n/a                     | —        | Updated extension guidance; the OSU pages read directly                                              | 2027-08-15                                  |
