...
// ---------------------------------------------------------------------------
// v1.2 / v1.3 — Recursive nested-output safety scanner
//
// The recursive scanner now lives in
// `src/test/utils/aiDoctorOutputSafetyScanner.ts` and is self-tested in
// `src/test/ai-doctor-output-safety-scanner.test.ts`. The golden suite
// only applies it across every fixture and renders one grouped report
// on failure.
// ---------------------------------------------------------------------------

describe("AI Doctor Golden Cases v1.3 — recursive output safety scan", () => {
  it("emits no unsafe phrases across every golden case (grouped report)", async () => {
    const all: ReturnType<typeof scanDiagnosisForUnsafePhrases> = [];
    for (const c of AI_DOCTOR_GOLDEN_CASES) {
      const r = await runCase(c);
      const found = scanDiagnosisForUnsafePhrases(r, { caseId: c.id });
      all.push(...found);
    }
    const report = formatUnsafePhraseReport(all);
