...
function resolveRpcCalls(source: string): Array<{ name: string; argsVar?: string }> {
  ...
  const clientCast = [...source.matchAll(CLIENT_CAST_RPC_CALL_PATTERN)].map((m) => ({
    name: m[1],
    argsVar: m[2],
  }));
...
describe("Action Queue complete/cancel transitions", () => {
  ...
  it("records status and audit through the canonical RPC (no privileged key)", () => {
    const rpcCallSiteCount =
      (src.match(/supabase\.rpc\b/g) ?? []).length +
      (src.match(/\(\s*supabase\s+as\s+unknown\s+as\s+\w+\s*\)\s*\.rpc\b/g) ?? []).length;
