...
BEGIN
  FOR role_name IN SELECT jsonb_object_keys(want) LOOP
    ...
    ASSERT overload_count > 0,
      'DIAGNOSTIC[missing-function]: create_feeding_event missing while checking EXECUTE overload matrix';
    ASSERT (
      expected AND granted_count = overload_count
    ) OR (
      NOT expected AND granted_count = 0
    ), format(
      'DIAGNOSTIC[genuine-permission-mismatch]: EXECUTE create_feeding_event for %I: expected all=%s, granted=%s/%s '
      '(one overload must not mask another; ACL disagrees with trust-boundary contract)',
