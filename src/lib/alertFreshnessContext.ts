 ...
 *     the LIVE window for every source, via `isStale(ts, now)`. Sensor Truth
 *     Canon keeps DISPLAY freshness source-aware (live 15m / manual 24h);
 *     alert persistence is deliberately tighter and must stay in lockstep
 *     with `isSnapshotPersistable` in `environmentAlertPersistence.ts`.
...
/** Alert-window freshness, in minutes, for operator-facing copy. */
 ...
 *
 * DELIBERATELY the LIVE window for every source. Sensor Truth Canon keeps
 * DISPLAY freshness source-aware (live 15m / manual 24h), but this module
 * mirrors the alert-engine persistence gate — and persisting an alert row
 * asserts the problem is happening NOW. Advertising a 24h manual window here
 * would promise "eligible for alert persistence" for snapshots that
 * isSnapshotPersistable rejects. Keep the two in lockstep.
 */
