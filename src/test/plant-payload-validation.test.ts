...
describe("validatePlantInsertPayload", () => {
  ...
  it("rejects an invalid grow_id", () => {
    expect(validatePlantInsertPayload({ ...base, grow_id: "not-a-uuid" }).ok).toBe(false);
  });

  it("keeps tent_id optional on a valid grow-bound payload", () => {
    const withoutTent = validatePlantInsertPayload(base);
    expect(withoutTent.ok).toBe(true);
    expect(withoutTent.value?.tent_id).toBeUndefined();

    const withTent = validatePlantInsertPayload({ ...base, tent_id: TENT_ID });
    expect(withTent.ok).toBe(true);
    expect(withTent.value?.tent_id).toBe(TENT_ID);
  });
...
describe("validatePlantRowResponse", () => {
  ...
  it("remains readable for legacy rows whose grow_id is null", () => {
    ...
    expect(r.ok).toBe(true);
    expect(r.value?.grow_id).toBeNull();
  });
});
