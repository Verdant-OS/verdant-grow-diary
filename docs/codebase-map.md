...
Called from the app with `supabase.functions.invoke(...)`. Re-measured at deploy tip
`28c01a01`, excluding test files: **17 call expressions across 15 production files** — 6 in
`src/hooks/`, 4 in `src/components/`, 3 in `src/lib/`, 2 in `src/pages/`. Hooks are the
largest single layer but a **minority**, so an edge-function audit that walks only
`src/hooks/` misses most consumers. Count `.functions.invoke(` call expressions, not path
mentions. Client-side entitlement reads are presentation-only; the `*-entitlement` functions are
...
| `.env.production`  | `VITE_PAYMENTS_CLIENT_TOKEN`                                                                                                                                                                           | Paddle **sandbox-class** client token — same class as development, not a live token. `src/lib/paddle.ts` states both builds carry sandbox-class tokens and that live tokens **fail closed on every host**; #1078 makes production checkout reject non-sandbox token classes before Paddle.js loads. Browser-side by design; still do not echo the value |
