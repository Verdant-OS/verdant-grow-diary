...
- `auto-tag-release.yml` runs on every push to `main` or `verdant-grow-diary` (`:15-17`) and tags
  ...
  the commit carries no tag yet**. A commit that already has any tag (manual, semver or an earlier
  run) exits successfully with neither a dated tag nor an annotation; only the run summary records
  its tree hash (`:58-72`). Tags exist for the last four deploy tips (Appendix A.7). An annotation
  is the fast path from a served `treeHash` back to a commit when a stamp has no git identity;
...
| M8  | Tag anchor for the tip                        | `git ls-remote --tags origin`, keeping lines whose SHA (the peeled `^{}` line for an annotated tag) is the tip's, once that push's `auto-tag-release.yml` run has finished. `PASS` when a tag points at the tip. The dated `v<yyyy>.<mm>.<dd>-<12-char SHA>` tag carries a `Tree-Hash:` annotation; a tag that pre-dated the run (`auto-tag-release.yml:58-72`) anchors the commit but carries none, so record that its `treeHash` link depends on the resolver's scan window (§5.1). No tag at the tip is `FAIL`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `NOT_MEASURED`                                                                        |
