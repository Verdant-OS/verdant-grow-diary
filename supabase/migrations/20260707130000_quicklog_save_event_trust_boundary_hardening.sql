-- QuickLog save-event trust-boundary hardening
--
-- Brings public.quicklog_save_event up to the contract asserted by
-- src/test/quicklog-save-event-rpc-trust-boundary.test.ts, which the prior
-- definition (20260701013256) predated. Four changes; nothing else about the
-- happy path or the already-validated rejection paths changes:
--
--   1. Pin search_path with the explicit quoted form: SET search_path TO
--      'public', 'pg_temp' (semantically identical to `= public, pg_temp`).
--   2. On save failure, RETURN a safe { ok:false, reason:'save_failed' }
--      envelope instead of RAISE. The BEGIN/EXCEPTION savepoint still rolls back
--      every partial write (spine/subtype/diary/idempotency), so there are no
--      orphans — but the caller now gets a clean result AND the 'save_failed'
--      audit row persists (RAISE previously rolled that audit back too, losing
--      it, and surfaced raw errors to the client).
--   3. Handle the concurrent idempotency race: if another transaction inserts
--      the same (user_id, idempotency_key) between our lookup and our insert,
--      the unique index raises unique_violation. That writer has committed (that
--      is why the index fired), so re-read its grow_event_id and collapse to the
--      same reused=true envelope as the explicit duplicate branch — no duplicate
--      spine/diary/idempotency rows.
--   4. Grant EXECUTE to authenticated + service_role only (drop anon — the
--      function is auth.uid()-gated, but the surface should be minimal).
--
-- Signature is unchanged, so CREATE OR REPLACE is safe. Verified against a
-- Supabase branch (happy path, idempotent replay, forced save_failed) and the
-- static trust-boundary suite; run the runtime RLS harness
-- (scripts/run-quicklog-rpc-rls-harnesses.ts) before release.
...
BEGIN
  ...
  BEGIN
    ...
    -- Subtype writes are atomic with the spine. Any trigger validation failure
    -- rolls back the whole save via the EXCEPTION handler below (no orphan
    -- spine, diary, or idempotency row) and returns a safe save_failed envelope.
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Concurrent idempotent replay: another transaction inserted the same
      -- (user_id, idempotency_key) between our lookup and our insert. It has
      -- committed (that is why the unique index fired), so re-read its
      -- grow_event_id and collapse to the same reused envelope — no duplicate
      -- spine/diary/idempotency rows. The failed inserts above are already
      -- rolled back to this block's implicit savepoint.
      SELECT grow_event_id INTO v_existing
        FROM public.quicklog_idempotency
       WHERE user_id = uid AND idempotency_key = p_idempotency_key;
      INSERT INTO public.quicklog_audit_events (user_id, idempotency_key, grow_event_id, status)
      ...
      RETURN jsonb_build_object('ok', true, 'grow_event_id', v_existing, 'reused', true);
    WHEN OTHERS THEN
      -- Partial writes are rolled back to this block's implicit savepoint. Log
      -- only SQLSTATE (the 5-char error code, never the raw message text) and
      -- return a safe envelope — do NOT RAISE, so the caller gets a clean result
      -- and this audit row persists.
      INSERT INTO public.quicklog_audit_events (user_id, idempotency_key, status, reason)
...
-- Grant EXECUTE to authenticated + service_role only (drop anon).
