...
export interface ResolveCreateTargetGrowResult {
  /** Resolved grow for write, or null when blocked. */
  targetGrowId: string | null;
  /**
   * True when page/default grow was non-empty but not in loaded grows
   * (and grows finished loading without error). Callers must not fall back.
   */
  requestedSetupUnavailable: boolean;
  /** Non-empty page default was supplied (may still be invalid). */
...
export function resolveCreateTargetGrowId(input: {
  ...
  if (input.growsLoading || input.growsError) {
    // Do not claim unavailable or fall back while unknown.
  ...
  if (page) {
    ...
    // Explicit request cannot be verified — never fall back to active.
...
/** @deprecated use buildCreateGrowBindingView — kept for gradual test migration */
export function buildCreateGrowBindingHardStop(
  ...
  // Synthesize grows list of correct length for legacy callers.
  const grows = Array.from({ length: Math.max(0, input.growCount) }, (_, i) => ({
    id: input.targetGrowId && i === 0 ? String(input.targetGrowId) : `synthetic-${i}`,
    name: `G${i}`,
  }));
  return buildCreateGrowBindingView(
    {
      pageDefaultGrowId: input.targetGrowId,
      activeGrowId: null,
      grows,
...
export interface SuppliedTentView {
  ...
  /** Tent id to keep in form state (may be pending id, never silently "none" for supplied). */
  tentId: string | null;
  blockSubmit: boolean;
  /** When true, grower must explicitly pick a compatible tent before write. */
...
export function evaluateSuppliedTentBinding(input: {
  ...
  tentsError?: boolean;
  /** Row for supplied tent once list is loaded; undefined while loading or not found. */
  ...
  if (!target) {
    // Grow not ready yet — keep supplied tent pending for write.
    return {
      kind: "pending",
      tentId: supplied,
      blockSubmit: true,
...
/**
 * Selected tent must match target when a tent is chosen.
 * When `suppliedTentRequiresSelection` and selection is "none", block submit.
 */
...
/**
 * Initial tent form value for plant create.
 * Supplied tent is preserved (pending) until known-compatible; never silent "none"
 * while a supplied tent is still required.
 */
export function resolveInitialPlantTentId(input: {
  ...
  // Keep supplied id in form for pending/unavailable/conflict so requireTent path stays armed.
  if (supplied.kind !== "none") return tentId;
  return "none";
}

/** Whether plant create may omit tent_id (generic create only). */
