...
function assertTelemetryNotClean(input: unknown, caseName: string): void {
  ...
  expect(row.status, `${caseName}: status must not be passed`).not.toBe(
    "passed",
  );
  expect(
  ...
  expect(
    hasUnsafeHealthyClaim(collectRowText(row)),
    `${caseName}: row text implies healthy`,
  ).toBe(false);
  ...
  expect(
    enriched.provenance,
    `${caseName}: provenance must not be direct`,
  ).not.toBe("direct");
  expect(
  ...
  expect(sensor.provenance, `${caseName}: loop sensor provenance`).not.toBe(
    "direct",
  );
  for (const r of rows) {
    expect(
  ...
  expect(
    hasUnsafeHealthyClaim(report),
    `${caseName}: text report implies healthy`,
  ).toBe(false);
}
...
describe("assertTelemetryNotClean helper — self-checks", () => {
  it("a genuinely fresh live snapshot is allowed to pass (guardrail sanity)", () => {
    ...
    const enriched = enrichLoopStepRow(row, baseEvidence({
      source: "live",
      captured_at: FRESH_ISO,
    }));
