...
promotion axis re-measured for Vercel Rolling Releases by Vercel reads from 15:21:47 to 15:50:20
UTC and GitHub Actions reads at 16:04:26–16:05:49 UTC (Appendix C). That amendment adds no
...
`rolling-release/start` request for the tip build at 15:50:03 UTC. Vercel refused it with `422`,
...
| --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLAUDE.md:130`                                                 | "**Lovable is the production publisher**"                                                                                  | `FAIL`         | **Corrected by `#1717`** (`ea361665`, merged 2026-09-26): the Platform row now calls the production publisher operating state. Twelve-file `Sentinel-Version` bump to `2026-09-25.1`                                 |
| `CLAUDE.md:138-144`                                             | "`vercel.json` does not govern production … Never reason about production redirect or header behaviour from `vercel.json`" | `FAIL`         | **Corrected by `#1717`** (`ea361665`): the paragraph now says never to assume the publisher. The durable form remains D-RT-5: a host file governs when the measured publisher applies it                             |
| `CLAUDE.md:77`                                                  | "`bun.lockb` authoritative"                                                                                                | `FAIL`         | **Corrected by `#1717`** (`ea361665`): `CLAUDE.md` now names `bun.lock` and says there is no `bun.lockb`                                                                                                             |
...
| Follow-up                                                                                                                                                                               | Kind                                                                          |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Governance correction: `CLAUDE.md:130` and `:138-144`, with the twelve-file `Sentinel-Version` bump                                                                                     | **Done**: merged as `ea361665` (`#1717`), 2026-09-26 15:49 UTC                |
...
- **The Rolling Releases amendment made one publish attempt, and it was refused.** Its session found
  ...
  `rolling-release/start` request for the tip build at 15:50:03 UTC. Vercel refused it with
...
sentinel_version: 2026-09-25.1 (the base since the e9639ac3 merge; the Appendix C reads began
  under 2026-09-01.5, before #1717 merged at 15:49 UTC)
date: 2026-09-26
...
verified_by:
  - Vercel reads from 15:21:47 to 15:50:20 UTC (Appendix C): get_rolling_release (opening,
    closing and post-attempt reads), get_rolling_release_config, get_runtime_errors (15:49:58),
  ...
  - GitHub: 41 open PR heads fetched and file-listed against their merge-bases with d510eb56;
    ci.yml merge_group runs (two pages, 30 runs) at 16:04:26–16:05:49 UTC for C.2's provenance
  - git: deploy-tip reads (fetch, then log) at 15:29:30–15:29:33 and 15:49:30–15:49:33 UTC;
    git log --first-parent at 16:04:15–16:04:21 UTC for C.2's tips
...
not_done:
  - One rolling-release start for the tip build was attempted after the owner's in-session grant,
    at 15:50:03 UTC. Vercel refused it (422) and nothing changed (C.7). No other rollout,
...
Every Vercel read below went through the Vercel tools attached to this session. Each was read-only
and made once. Times are UTC, from the session's tool log, subagents included, truncated to the
second: a range runs from when the request was sent to when its result came back. Deployment IDs are
truncated to 12 characters. The event log returned email addresses, token identifiers and session
identifiers, and none of them is recorded here (AT-12).

### C.1 The rolling-release record and configuration — M10 (rollout first), 15:26:49–15:26:52 UTC
...
| Re-read at 15:29:29–15:29:31, after C.2's 15:27 build was `READY` | Unchanged: `COMPLETE`, the same canary, `queuedDeploymentId: null`                                                                                                                                                                                                                                                                     |
| Closing read at 15:49:31–15:49:35 UTC, after C.4's hostname reads | Rollout record unchanged (`COMPLETE`, same canary, same `updatedAt` 06:21:02); deploy tip unchanged (`d510eb56`, fetched and read at 15:49:30–15:49:33). Hostnames, inventory and DNS were not re-read, so this is only a partial bracket. C.4's `FAIL` needs no bracket under M10, but this appendix could not have reported a `PASS` |

### C.2 Deploy-branch production builds since 00:00 — M4, `list_deployments`, 15:29:22–15:29:24
...
- **Tips.** `git log --first-parent --since=2026-09-25T23:00Z`, run at 16:04:15–16:04:21 UTC, lists
  exactly these nine commits, so no tip lacks a build row. The earlier start takes in `53707822`,
  which was committed at 23:22 and built at 01:47.
- **Provenance.** It was read from the Actions runs API (`ci.yml`, `event=merge_group`) in two page
  reads, at 16:04:26–16:04:28 and 16:05:48–16:05:49 UTC. The two pages read, 30 runs, cover every
  such run from 2026-09-24 09:07 to 2026-09-26 15:55 UTC without a gap. Seven tips are the
  `head_sha` of a successful run on a `gh-readonly-queue/verdant-grow-diary/pr-<N>-<parent>` branch.
  `4ddb2322` and `408c966c` are the `head_sha` of none. The PR records show both `#1703` and `#1714`
  merged directly by the owner's account, three to four seconds before their builds were created.
...
The read ran at 15:28:51–15:28:54 UTC on 2026-09-26. The request's `until` of 15:40 was later than
the read, so nothing after 15:28:54 is covered. Eleven type names were requested and six events came
back.
...
15:27:33–15:27:37 UTC. It returned 40 events, which is the request's limit, so
...
### C.4 What each production hostname served — M10, one read-only pass, 15:29:29–15:30:06 UTC
...
- **Order and times.** One read-only pass. The inventory, hostname and alias reads ran in a
  subagent.
  - Opening rollout read, `get_rolling_release`: 15:29:29–15:29:31 (C.1).
  - Opening deploy tip, `git fetch` then `git log`: 15:29:30–15:29:33, `d510eb56`.
  - `list_project_domains`: 15:29:55–15:29:57.
  - `get_deployment` for the four hostnames, in table order: 15:29:59–15:30:04.
  - `list_promote_aliases`: 15:30:04–15:30:06.
  - Closing reads: rollout record and tip only, at 15:49 (C.1), so the bracket is partial.
...
- **Release state: `FAIL` (D-RT-12), in its "built, not rolled out" form.** It rests on the two
  platform-owned hostnames, which served `408c966c`, not the tip `d510eb56`. They were compared
  with the opening tip, read before them. `408c966c` is two first-parent commits behind it, and
  the closing tip was still `d510eb56`, so no merge landed during the run. The tip's `READY`
...
| When         | 15:50:03–15:50:05 UTC on 2026-09-26, request sent to response                                                                                                                                                                                                                                                                                                                     |
...
| Checks first | The tip was `d510eb56`, and its `merge_group` run was `success`. The build was `READY`, production and git. The rollout record was `COMPLETE` with none active. Vercel `get_runtime_errors` (project, `since: 1h`), read at 15:49:58–15:50:00 UTC just before the request, returned no runtime errors for the preceding hour. `408c966c`..`d510eb56` changes nothing under `src/` |
| Reason       | Close the "built, not rolled out" `FAIL` with a build whose application code equals the served one                                                                                                                                                                                                                                                                                |
| Outcome      | `422 unprocessable`: "Unable to start rolling release for this canary deployment." The rollout record read straight afterwards (`get_rolling_release`, 15:50:18–15:50:20 UTC) was unchanged: `COMPLETE`, canary `dpl_FsmTjonj…`, `queuedDeploymentId: null`                                                                                                                       |
