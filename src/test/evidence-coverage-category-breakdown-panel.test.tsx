...
describe("EvidenceCoveragePanel — category breakdown", () => {
  it("renders the Coverage by category heading and subcopy", () => {
    ...
    expect(screen.getByRole("heading", { name: /coverage by category/i })).toBeInTheDocument();
    expect(screen.getByText(/grouped counts only/i)).toBeInTheDocument();
  });
  ...
  it("renders alert category rows", () => {
    ...
    render(<EvidenceCoveragePanel viewModel={vm} />);
    expect(screen.getByTestId("evidence-coverage-alerts-by-category-row-vpd")).toBeInTheDocument();
    expect(screen.getByTestId("evidence-coverage-alerts-by-category-row-temp")).toBeInTheDocument();
  });
  ...
  it("renders action category rows", () => {
    ...
    render(<EvidenceCoveragePanel viewModel={vm} />);
    expect(
      screen.getByTestId("evidence-coverage-actions-by-category-row-adjust_vpd"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("evidence-coverage-actions-by-category-row-check_runoff"),
    ).toBeInTheDocument();
  });
  ...
  it("renders empty-state copy when no categories", () => {
    const vm = buildEvidenceCoverageViewModel({ alerts: [], actions: [] });
    render(<EvidenceCoveragePanel viewModel={vm} />);
    expect(screen.getByText(/no alert categories to summarize yet/i)).toBeInTheDocument();
    expect(screen.getByText(/no action categories to summarize yet/i)).toBeInTheDocument();
  });
  ...
  it("never renders raw refs, payloads, tokens, prompts, or automation/healthy claims", () => {
    const vm = buildEvidenceCoverageViewModel({
      ...
      actions: [{ action_type: "adjust_vpd", originating_timeline_events: [] }],
    });
