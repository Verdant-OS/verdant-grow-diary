...
- **Safety/professional referral — mandatory, not optional.** Compressed or generated
  ...
  **Each authority defines its own averaging period — they are not interchangeable:**
  ...
  | Cal/OSHA PEL         | 5,000 ppm               | up to 10-hour TWA                         | 30,000 ppm (ST)         | Enforceable in California |
...
4. **Four sources returned HTTP 403 to this author** (S5, the OSU half of S6, and the S9
   publisher page). They are marked NOT READ in the register rather than presented as read.
   **C07 must not be published on this draft's authority**, and S9 is cited from its
   institutional record and search summaries rather than its full text — an evidence editor
   must obtain it before C04a/C04b advance.
