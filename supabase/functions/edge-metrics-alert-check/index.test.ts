...
Deno.test(
  "postWebhook: retries transient 5xx with exponential backoff, then succeeds",
  async () => {
    const origFetch = globalThis.fetch;
    let calls = 0;
...
Deno.test(
  "postWebhook: exhausted transient retries mark gave_up_transient and log retries",
  async () => {
    const origFetch = globalThis.fetch;
    const origWarn = console.warn;
