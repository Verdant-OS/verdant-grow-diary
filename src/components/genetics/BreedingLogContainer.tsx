...
export function BreedingLogContainer({ activeGrowId, plants, onCreated, onCancel }: Props) {
  ...
  const handleSubmit = async (data: { plantId: string; subType: BreedingEventType; details: any }) => {
    setBusy(true);
    try {
      ...
      try {
        const { data: fnData, error: fnError } = await supabase.functions.invoke("create-breeding-suggestions", {
          body: { event_id: eventRow.id },
        });
        if (fnError) {
          console.error("[BreedingLogContainer] Edge function error:", fnError);
        } else {
          const actionIds = (fnData as { actionIds?: Array<{ id: string; plantId: string | null }> } | null)?.actionIds ?? [];
