...
prerequisite is a safety envelope on 15 harnesses, and that is a security change needing one owner
...
**Do not append the 16 unrun harnesses to `test:security-db-local`.** The audit
(`docs/audits/test-coverage-audit-2026-08-29.md`, finding F3) correctly measured that 16 of 33
runtime harnesses are invoked by no workflow. It did not establish _why_, and the why changes the
slice completely.

`established fact`: of those 16, exactly **one** — `scripts/run-genetics-propagation-rls-harness.ts`
...
join test:security-db-local"_). The other 15 have no envelope to speak of.
...
## 3. Measured envelope coverage across the 16
...
**P5.1 — envelope, no wiring.** Give each of the 15 the four properties above, preferring
`assertSupabaseDatabaseTargetIdentity` over a fifteenth hand-rolled ref check. Add a static test per
...
**P5.2 — one static guard.** A single test asserting every `scripts/*harness*` file carries the
envelope, so a sixteenth harness cannot land without one. This is the P2(d) pattern applied to
...
`established fact`: all 16 are recorded in the P2(d) execution manifest
