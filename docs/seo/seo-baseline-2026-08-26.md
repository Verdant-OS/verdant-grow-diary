...
This file is the Day-0 successor to [`docs/seo/seo-baseline.md`](seo-baseline.md) (2026-07-30, `1c40c21f2`). That file is preserved unchanged as the historical snapshot; this file does not overwrite or restate it.
...
| vs. tip       | Live ≠ tip; **publish lag NOT COMPUTABLE** (the brief, written 2026-08-26, cited the then-current tip #1156 = `2cc97c0e` — not the branch state as of the 2026-08-25 build) | **Live = tip.** Served commit equals `origin/verdant-grow-diary` head — |
|               |                                                                                                                                                                             | publish lag **0 commits at 21:49:48Z**                                  |

`established fact` — production **republished between the two readings**: the build served on 2026-08-26 was stamped 21:44:08Z, roughly five minutes before this sweep began. The 2026-08-25 identity FAIL (live ≠ tip) stands as recorded about the 2026-08-25 build — this sweep neither re-verifies nor contradicts it — and it does **not** describe the build served at this baseline's timestamps.
...
**P0 private-content leak: PASS** on the unauthenticated sweep set (both readings — brief 2026-08-25, and this sweep 2026-08-26). `established fact` for 2026-08-26: an automated scan of all 65 fetched bodies found zero occurrences of `service_role`, zero JWT-shaped tokens, zero live-secret-class strings, and zero non-`verdantgrowdiary.com` email addresses. Scope bound: this proves nothing about authenticated surfaces; `/grows` and `/operator` were not followed.
...
| --- | ------------------------------------------------------------ | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Live serves the deploy-branch tip                            | FAIL (live ≠ tip; lag not computable)                      | **PASS at timestamp** — live = tip `2cc97c0e91aa`, lag 0. Provenance is a separate, non-passing sub-check: `__orphan__` persists, cause NOT_MEASURED.                                                    |
...
**Gate Zero is not all true** at either reading — consistent with the sprint brief. Conditions 3, 4, and 6 fail as of 2026-08-26T21:53Z; conditions 1 and 2 pass **only at their timestamps** and are perishable.

## Operating rule

This file is the Day-0 comparison point for the Organic Knowledge-to-Diary sprint. It does **not** authorize a content cohort, a publish, or any production change — those decisions belong to GDP/Cheek, gated on Gate Zero actually holding. Any future measurement that differs from a row here gets its own timestamped entry (in a successor file or `docs/agents/CURRENT_STATE.md`); rows in this file are never edited to match later reality.
