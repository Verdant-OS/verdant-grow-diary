...
const Email = ({
  ...
  <EmailLayout preview={`Your ${productName} order is confirmed.`}>
    <Text style={styles.h1}>{firstName ? `Thanks, ${firstName}.` : "Thanks for your order."}</Text>
    <Text style={styles.p}>
      Your purchase of <strong>{productName}</strong> is confirmed. Access is already active on your
      account — no activation steps needed.
    </Text>
    ...
    <Text style={{ ...styles.p, marginTop: "20px" }}>
      If anything looks off — wrong plan, wrong amount, duplicate charge — reply to this email and
      I&rsquo;ll fix it personally.
    </Text>
    {signOff()}
  </EmailLayout>
);
