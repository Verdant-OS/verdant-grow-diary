/**
 * Static-safety guard for V0 harvest + cure_check Quick Log slice.
 *
 * Asserts the new files contain no imports or wording that would violate
 * the V0 safety rules (no AI, no alerts persistence, no Action Queue
 * writes, no automation/device-control, no service_role leakage).
 */
import { describe, it, expect } from "vitest";
import { readFileSync } from "node:fs";

const FILES = [
  "src/constants/quickLogEventTypes.ts",
  "src/lib/harvestCureRules.ts",
  "src/lib/harvestCureTimelineCardViewModel.ts",
];
