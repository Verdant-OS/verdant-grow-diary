...
const FILES = ["src/lib/sensorSnapshotFreshnessRules.ts", "src/components/SensorSnapshotCard.tsx"];
  ...
  for (const rel of FILES) {
    it(`${rel} contains no forbidden patterns`, () => {
      ...
      for (const pattern of FORBIDDEN) {
        expect(pattern.test(src), `Forbidden pattern ${pattern} found in ${rel}`).toBe(false);
      }
    });
  }
});
