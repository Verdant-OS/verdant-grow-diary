...
describe("formatManualSensorDisplayValue", () => {
  it("keeps Fahrenheit as the backward-compatible default", () => {
    expect(formatManualSensorDisplayValue("temp_f", 77)).toEqual({
      magnitude: "77",
      unit: "°F",
    });
  });

  it("converts only the displayed temperature when Celsius is selected", () => {
    const snapshot = buildFreshnessSnapshot("temp_f", { value: 77, loggedAt: hoursAgo(1) }, NOW);

    expect(formatManualSensorDisplayValue("temp_f", snapshot.value, "celsius")).toEqual({
      magnitude: "25",
      unit: "°C",
    });
    expect(snapshot.value).toBe(77);
  });

  it("returns null for missing or non-finite display values", () => {
    expect(formatManualSensorDisplayValue("temp_f", null, "celsius")).toBeNull();
    expect(formatManualSensorDisplayValue("temp_f", Number.NaN, "fahrenheit")).toBeNull();
  });
});
