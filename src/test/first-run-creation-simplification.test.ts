...
describe("CreateTentDialog — first-run simplification", () => {
  ...
  it("shows 'Enrich later' guidance copy", () => {
    ...
    expect(CREATE_TENT).toMatch(/Verdant works best\s+once your first\s+plant memory exists\./);
  });
...
describe("CreatePlantDialog — first-run simplification", () => {
  ...
  it("shows 'Enrich later' guidance copy", () => {
    ...
    expect(CREATE_PLANT).toMatch(/Verdant works best\s+once your first\s+plant memory exists\./);
  });
