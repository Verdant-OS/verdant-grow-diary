// Pure, read-only EcoWitt → Seedling Tent normalizer.
//
...
export const SEEDLING_TENT_LABEL = "Seedling Tent" as const;

export const SEEDLING_TENT_CHANNEL_MAP = {
  air_temp_f: "temp2f",
  humidity_pct: "humidity2",
} as const;

export type SeedlingTentChannelMap = typeof SEEDLING_TENT_CHANNEL_MAP;

export function normalizeEcowittSeedlingTentPayload(
  ...
  const airRaw = toNumberOrNull(payload[SEEDLING_TENT_CHANNEL_MAP.air_temp_f]);
  ...
  const root_zone_confidence: RootZoneConfidence = "missing";
  ...
  return {
    ...
    tent_label: SEEDLING_TENT_LABEL,
    ...
    channel_map: SEEDLING_TENT_CHANNEL_MAP,
