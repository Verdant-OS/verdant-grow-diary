...
There is no fixture rewrite or automatic fixture reset. Every Quick Log save
the smoke performs is a real, persisted diary entry, exactly like a grower
clicking Save.
...
- **Use only a dedicated test account and a dedicated test plant.** The
  ...
  care about.
- **Run the workflow manually only.** Write-producing smoke against a real
  grow is unsafe and is not enabled.
- No automatic fixture reset or mutation of existing grow data happens outside
  the intentional Quick Log save flow itself.

## Safety guarantees

- Real `/auth` UI only.
- No hardcoded credentials in the repo.
- No elevated DB-role or bridge tokens in browser context.
...
- The GitHub Actions workflow runs only through manual `workflow_dispatch`.
...
| `E2E_GROW_1_PLANT_URL`  | Full URL of the disposable E2E plant detail page     |
| `E2E_FIXTURE_MODE`      | Must be exactly `true`                               |
| `E2E_FIXTURE_EXPECTED_TENT_NAME` | Required. Must be `E2E Test Tent`          |
| `E2E_FIXTURE_EXPECTED_PLANT_NAME` | Required. Must be `E2E Test Plant`        |
| `E2E_FIXTURE_EXPECTED_GROW_NAME` | Optional/future only                         |
...
- reads the current page and expected fixture names only
- never overwrites grow/tent/plant names
- relies on the normal storageState produced by `auth.setup.ts`

The smoke writes real diary entries into the E2E fixture. No automatic
fixture reset is performed; periodically prune the E2E Test Plant's diary
manually if desired. The smoke workflow is manual only.

An **optional** UI-only bootstrap is available behind an explicit
opt-in flag (`E2E_ALLOW_FIXTURE_BOOTSTRAP=true`). It is **off by
default**, never renames or overwrites data, and refuses to force
creation when stable selectors are missing. Bootstrap remains **deferred** for general use — see the
...
admin APIs, and never writes app data.
...
- Never renames or modifies existing grows/tents/plants.
...
- A disposable E2E plant detail URL you control via the test account.
...
project skips with a clear message. It does not use an alternate auth path.
...
- `workflow_dispatch` only. Manual runs from branch `verdant-grow-diary` fail
  fast with a clear message if any required secret/var is missing:
  ```
  Missing required Quick Log smoke configuration.
  ```
...
- Variables:
  ...
  - `E2E_FIXTURE_EXPECTED_PLANT_NAME=E2E Test Plant`
- Optional variable:
  - `E2E_FIXTURE_EXPECTED_GROW_NAME` (future only; the current setup flow has
    no visible Grow page)
  - `E2E_FIXTURE_EXPECTED_ACCOUNT_HINT`
  - `E2E_GROW_2_PLANT_NAME` (defaults to `"505 Headbanger"`)

Run order:

1. Verify required config names.
2. Install dependencies.
3. Run `bun run e2e:fixture-checklist -- --ci`.
4. Install Playwright Chromium.
5. Optionally run `bun run e2e:bootstrap-fixture` only when explicitly enabled.
6. Run `bun run e2e:verify-fixture`.
7. Run `bun run e2e:quicklog-smoke` only after fixture verification succeeds.
...
- Cache `path:` values are restricted to `~/.bun/install/cache` and
  ...
  privileged Supabase credentials) are never cached.
...
Quick Log smoke is manual-dispatch only. Write-producing smoke must only
run against a disposable test account/test plant. See the real-write warning
...
- **Missing GitHub variable or secret**
  ...
  - Fix: add the missing repo Actions vars/secrets
    (`E2E_BASE_URL`, `E2E_GROW_1_PLANT_URL`, `E2E_FIXTURE_MODE=true`,
    `E2E_FIXTURE_EXPECTED_TENT_NAME`, `E2E_FIXTURE_EXPECTED_PLANT_NAME`,
...
- **Cannot find disposable plant detail page**
...
- **Cannot find optional target plant**
