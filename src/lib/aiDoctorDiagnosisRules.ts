...
export function validateAndSanitizeDiagnosis(input: unknown): SanitizeReport {
  ...
  const followUp24h = coerceFollowUp(input.followUp24h, CAUTIOUS_FALLBACK.followUp24h);
  const recoveryPlan3d = coerceFollowUp(input.recoveryPlan3d, CAUTIOUS_FALLBACK.recoveryPlan3d);
