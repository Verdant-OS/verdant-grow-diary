...
| **C — Pi** | `pi-ingest-readings` | HMAC headers: `x-bridge-id`, `x-bridge-signature`, `x-bridge-timestamp`; canonical request signature and tent scope described below | Yes, if HMAC + entitlement pass |
| **D — EcoWitt validate-only** | `ecowitt-real-ingest` | Static env secret `ECOWITT_BRIDGE_TOKEN` | **No** — validate only |

#### Pi signing contract (canonical)

The Pi lane does **not** sign only the body. It computes HMAC-SHA256 over this exact UTF-8 string:
...
1. resolves `x-bridge-id` to an active server-side credential;
2. rejects timestamps more than **5 minutes** old or more than **5 minutes** in the future;
3. requires the request tent and checks it against that bridge credential’s allowed tent ids;
4. builds the signing string from the actual HTTP method, request path, header timestamp, and exact raw body bytes/string;
5. compares the lowercase HMAC hex with `x-bridge-signature`;
6. derives owner and tent identity from the verified credential, never from an untrusted user id in the body.

A bridge implementation that signs a re-serialized JSON body, omits method/path/timestamp, or sends a tent outside its allowlist must fail authentication.
