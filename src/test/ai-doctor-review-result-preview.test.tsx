...
describe("AiDoctorReviewResultPreview", () => {
  it("renders the exact empty state when no result", () => {
    ...
    expect(screen.getByTestId("ai-doctor-review-result-empty-state").textContent).toBe(
      "No AI Doctor review result yet.",
    );
    expect(screen.getByTestId("ai-doctor-review-result-preview").getAttribute("data-state")).toBe(
      "empty",
    );
  });

  it("renders empty state with no partial content for invalid input", () => {
    render(<AiDoctorReviewResultPreview result={{ ...valid(), confidence: "bogus" }} />);
    expect(screen.getByTestId("ai-doctor-review-result-preview").getAttribute("data-state")).toBe(
      "empty",
    );
  ...
  it("renders summary, confidence, risk, evidence, missing, actions, follow-ups for a valid result", () => {
    ...
    expect(screen.getByTestId("ai-doctor-review-result-preview-label").textContent).toBe(
      "Review result preview — no AI request sent.",
    );
    expect(screen.getByTestId("ai-doctor-review-result-summary").textContent).toMatch(/leaf curl/);
    expect(screen.getByTestId("ai-doctor-review-result-confidence").textContent).toMatch(
      /Medium confidence/,
    );
    expect(screen.getByTestId("ai-doctor-review-result-risk").textContent).toMatch(/Watch/);
    expect(screen.getByTestId("ai-doctor-review-result-evidence").textContent).toMatch(
      /Tent temp 29C/,
    );
    expect(screen.getByTestId("ai-doctor-review-result-missing").textContent).toMatch(/VPD/);
    expect(screen.getByTestId("ai-doctor-review-result-immediate-action").textContent).toMatch(
      /Lower tent temperature/,
    );
    expect(screen.getByTestId("ai-doctor-review-result-what-not-to-do").textContent).toMatch(
      /Do not increase/,
    );
    expect(screen.getByTestId("ai-doctor-review-result-follow-up").textContent).toMatch(/24/);
  });
  ...
  it("renders action_queue_suggestion as approval-required preview only", () => {
    ...
    expect(screen.getByTestId("ai-doctor-review-result-suggestion-title").textContent).toMatch(
      /lowering tent target temperature/,
    );
    expect(screen.getByTestId("ai-doctor-review-result-suggestion-notice").textContent).toBe(
