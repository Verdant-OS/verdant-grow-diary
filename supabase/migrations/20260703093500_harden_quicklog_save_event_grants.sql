-- Re-harden quicklog_save_event EXECUTE grants: authenticated only.
--
-- Migration 20260701013256 recreated quicklog_save_event with the typed
-- payload signature and restored broad grants (anon, authenticated,
-- service_role). This RPC is a user-scoped WRITE path (grow_events +
-- companion diary rows) and must not be executable by anon; the function
-- already rejects unauthenticated callers via auth.uid(), and the grant
-- surface must match (defense in depth, per the trust-boundary contract
-- in src/test/quicklog-save-event-rpc-trust-boundary.test.ts).
--
-- The function is re-declared VERBATIM from 20260701013256 (same
-- signature, same body, no behavior change); only the search_path syntax
-- is normalized to the quoted form and the grants are tightened back to
-- the 20260626213951 posture: EXECUTE for authenticated only.
...
-- Tighten grants back to authenticated-only (matches 20260626213951).
