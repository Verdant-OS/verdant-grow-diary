...
| Topology: gateway → listener → `sensor-ingest-webhook` only (topology record §2)                                                                                                         | Incomplete — but only about the protocol, not the transport. A second deployed Edge function, `supabase/functions/ecowitt-ingest`, speaks the gateway's custom-upload **protocol** (parses the form, maps PASSKEY-fingerprint → tent via `hardware_config`) — yet it too requires `Authorization: Bearer` (bridge token, `index.ts:208-209` on deploy) and 401s without it. Since the Customized-server UI cannot set headers (§1 of the topology record), **an auth-capable bridge/listener hop is mandatory on every path**; no deployed endpoint serves the gateway directly. The listener/webhook path is the Windows-testbench route |
...
- The deployed `ecowitt-ingest` function (bearer-authenticated; reachable only via a
  bridge hop, never by the gateway alone) historically wrote the non-canonical literal
