...
export const CUSTOMER_GUIDE_TRUST_TELEMETRY_COPY = "This page is not live sensor telemetry.";
...
export const CUSTOMER_GUIDE_TRUST_FAQ: ReadonlyArray<TrustFooterFaqItem> = [
  ...
  },
  {
    ...
    answer: "No. " + CUSTOMER_GUIDE_TRUST_PRIVACY_COPY,
  },
  {
    id: "are_operator_notes_shown",
    question: "Are operator notes shown?",
    answer: "No. Operator notes stay with the grower and are never published to this page.",
  },
  {
...
export default function CustomerGuideTrustFooter() {
  return (
    ...
    >
      <h2 id="customer-guide-trust-heading" className="text-base font-semibold tracking-tight">
        Privacy and FAQ
      </h2>

      <div className="mt-3 space-y-2">
        <p data-testid="customer-guide-trust-privacy" className="text-sm text-muted-foreground">
          {CUSTOMER_GUIDE_TRUST_PRIVACY_COPY}
        </p>
        <p data-testid="customer-guide-trust-telemetry" className="text-sm text-muted-foreground">
          {CUSTOMER_GUIDE_TRUST_TELEMETRY_COPY}
        </p>
        <p data-testid="customer-guide-trust-backend" className="text-xs text-amber-300/80">
      ...
      <dl className="mt-5 space-y-4">
        {CUSTOMER_GUIDE_TRUST_FAQ.map((item) => (
          <div key={item.id} data-testid={`customer-guide-trust-faq-${item.id}`}>
